...
fn bench_rasterization_deathmatch(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("pixelflow_cached", "64px_g"), |b| {
        b.iter(|| {
            rasterize(&cached_colored, &mut frame, 1);
            black_box(&frame);
        })
    });
    ...
    group.bench_function(BenchmarkId::new("pixelflow_parallel_2", "64px_g"), |b| {
        b.iter(|| {
            rasterize(&cached_colored, &mut frame, 2);
            black_box(&frame);
        })
    });
    ...
    group.bench_function(BenchmarkId::new("pixelflow_parallel_4", "64px_g"), |b| {
        b.iter(|| {
            rasterize(&cached_colored, &mut frame, 4);
            black_box(&frame);
        })
    });
