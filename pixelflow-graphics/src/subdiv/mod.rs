...
///
/// For (u, v) in tile depth n, computes scale = 2^n such that
/// (u * scale, v * scale) lands in the first tile [0.5, 1]².
///
/// Formula: depth = max(0, -floor(log2(max(u, v))) - 1), scale = 2^depth
///
/// Special cases:
/// - (u, v) in [0.5, 1]²: depth = 0, scale = 1.0 (first tile)
/// - (u, v) in [0.25, 0.5)²: depth = 1, scale = 2.0 (second tile)
/// - (u, v) in [0.125, 0.25)²: depth = 2, scale = 4.0 (third tile)
/// - At origin: clamp prevents infinity
#[inline]
fn tile_scale() -> impl Manifold<Output = Field> {
    // Beautiful, idiomatic kernel! macro composition
    // Computes: depth = max(0, -floor(log2(max(u, v))) - 1), scale = 2^depth
    kernel!(|| {
        let max_uv = X.max(Y).max(1e-10);
        let depth = max_uv.log2().floor().neg() - 1.0;
        depth.max(0.0).exp2()
    })()
}
...
mod tests {
    ...
    fn eval_scalar<M: Manifold<Output = Field>>(m: &M, x: f32, y: f32) -> f32 {
        ...
        // Debug: also directly eval to see the Field value
        let direct_field = bound.eval((Field::from(0.0), Field::from(0.0), Field::from(0.0), Field::from(0.0)));
        println!("DEBUG eval_scalar({}, {}): Field = {:?}, tex[0] = {}", x, y, direct_field, tex.data()[0]);
    ...
    fn test_tile_scale_values() {
        ...
        // Debug: test intermediate values
        let m1 = eval_scalar(&X.max(Y).max(1e-10f32), 0.75, 0.75);
        let l1 = eval_scalar(&X.max(Y).max(1e-10f32).log2(), 0.75, 0.75);
        let f1 = eval_scalar(&X.max(Y).max(1e-10f32).log2().floor(), 0.75, 0.75);
        let n1 = eval_scalar(&X.max(Y).max(1e-10f32).log2().floor().neg(), 0.75, 0.75);

        println!("At (0.75, 0.75):");
        println!("  max_uv = {}", m1);
        println!("  log2(0.75) = {} (expected: {})", l1, 0.75f32.log2());
        println!("  floor = {}", f1);
        println!("  neg(floor) = {}", n1);
        println!("  neg(floor) - 1 = {}", n1 - 1.0);
        println!("  max(0, neg(floor) - 1) = {}", (n1 - 1.0).max(0.0));
        println!("  scale = 2^... = {}", 2.0f32.powf((n1 - 1.0).max(0.0)));

        // Test log2 with simple values
        let test_0_5 = eval_scalar(&Field::from(0.5).log2().constant(), 0.0, 0.0);
        let test_2_0 = eval_scalar(&Field::from(2.0).log2().constant(), 0.0, 0.0);
        let test_1_0 = eval_scalar(&Field::from(1.0).log2().constant(), 0.0, 0.0);
        println!("\nlog2 test values:");
        println!("  log2(0.5) = {} (expected: -1.0)", test_0_5);
        println!("  log2(1.0) = {} (expected: 0.0)", test_1_0);
        println!("  log2(2.0) = {} (expected: 1.0)", test_2_0);

        // Test X directly
        let test_x = eval_scalar(&X, 0.75, 0.0);
        let test_x_log2 = eval_scalar(&X.log2(), 0.75, 0.0);
        println!("\nDirect X tests:");
        println!("  X at (0.75, 0) = {}", test_x);
        println!("  X.log2() at (0.75, 0) = {} (expected: {})", test_x_log2, 0.75f32.log2());

        // Test if the issue is with the evaluation method
        let test_const_log2 = eval_scalar(&0.75f32.log2().constant(), 0.0, 0.0);
        println!("  constant 0.75.log2() = {}", test_const_log2);
