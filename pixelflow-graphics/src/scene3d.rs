...
///
/// **Optimized Implementation**: Performs early exit if mask is fully true or fully false.
#[derive(Clone, Copy, ManifoldExpr)]
    ...
    B: ManifoldCompat<Jet3, Output = Field>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        // 2. Validate hit
        // t.val > 0.0 && t.val < 1000000.0
        // derivatives magnitude squared < 10000.0^2
        let t_val = t.val;
        let zero = Field::from(0.0);
        ...
        let dy = t.dy;
        let dz = t.dz;
        // Evaluate AST to Field
        let deriv_sq = (dx * dx + dy * dy + dz * dz).eval(p);
        let deriv_max_sq = Field::from(10000.0 * 10000.0);
        let valid_deriv = deriv_sq.lt(deriv_max_sq);

        let mask = valid_t & valid_deriv;

        // OPTIMIZATION: Early exit
        // If no lanes hit, return background immediately
        if !mask.any() {
        ...
        // If all lanes hit, return material immediately
        if mask.all() {
            return self.material.eval_raw(hx, hy, hz, w);
        }

        // 4. Sample both (mixed case) and blend
