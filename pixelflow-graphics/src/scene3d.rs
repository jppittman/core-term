...
const CHECKER_PATTERN_THRESHOLD: f32 = 0.25;
const CHECKER_AA_EPSILON: f32 = 0.001;
const SKY_R_BASE: f32 = 0.7;
const SKY_R_SCALE: f32 = 0.5;
const SKY_G_BASE: f32 = 0.85;
const SKY_G_SCALE: f32 = 0.45;
const SKY_B_SCALE: f32 = 0.2;
...
impl<M: ManifoldCompat<Jet3, Output = Field>> Manifold<Jet3_4> for Reflect<M> {
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let two = Jet3::constant(Field::from(REFLECT_CURVATURE_SCALE));
        let k = two * d_dot_n;
...
impl<C: ManifoldCompat<Field, Output = Discrete>> Manifold<Jet3_4> for ColorSky<C> {
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let r = (Field::from(SKY_R_BASE) - t * Field::from(SKY_R_SCALE)).constant();
        let g = (Field::from(SKY_G_BASE) - t * Field::from(SKY_G_SCALE)).constant();
        let b = (one - t * Field::from(SKY_B_SCALE)).constant();
...
impl<C: ManifoldCompat<Field, Output = Discrete>> Manifold<Jet3_4> for ColorChecker<C> {
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let is_even = fract_half.abs().lt(Field::from(CHECKER_PATTERN_THRESHOLD));
