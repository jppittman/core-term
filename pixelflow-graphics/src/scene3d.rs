...
/// Epsilon for sphere intersection to prevent self-intersection.
pub const SPHERE_EPSILON: f32 = 0.0001;

/// Epsilon for normal length to prevent division by zero.
pub const REFLECT_NORMAL_EPSILON: f32 = 1e-10;

/// Minimum cosine of incidence for reflection curvature scaling (prevents infinity).
pub const REFLECT_MIN_COS_INCIDENCE: f32 = 0.1;

/// Curvature scale factor for reflection.
pub const REFLECT_CURVATURE_SCALE: f32 = 2.0;
...
impl<M: ManifoldCompat<Jet3, Output = Field>> Manifold<Jet3_4> for Reflect<M> {
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let curvature_scale =
            (Field::from(REFLECT_CURVATURE_SCALE) / cos_incidence).constant();
