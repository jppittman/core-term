...
/// Epsilon for sphere intersection t calculation.
pub const SPHERE_EPSILON: f32 = 0.0001;

/// Epsilon for normal vector normalization to avoid division by zero.
pub const REFLECT_NORMAL_EPSILON: f32 = 1e-10;

/// Minimum cosine of incidence angle for reflection curvature scaling.
pub const REFLECT_MIN_COS_INCIDENCE: f32 = 0.1;

/// Curvature scaling factor for reflection.
...
impl<M: ManifoldCompat<Jet3, Output = Field>> Manifold<Jet3_4> for Reflect<M> {
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let inv_n_len = n_len_sq.max(Field::from(REFLECT_NORMAL_EPSILON)).sqrt().rsqrt();
