    ...
    B: ManifoldCompat<Jet3, Output = Field>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let t_val = t.val;
        let valid_t = (t_val.gt(Field::from(0.0)) & t_val.lt(t_max)).eval(p);

        // dx, dy, dz are Fields (derivatives of t with respect to screen coords)
        let dx = t.dx;
        let dy = t.dy;
        let dz = t.dz;
        let deriv_mag_sq = dx * dx + dy * dy + dz * dz;
        let valid_deriv = deriv_mag_sq.lt(deriv_max_sq).eval(p);

        let mask = valid_t & valid_deriv;

        // 3. Optimization: Early exit if all miss (background only)
        if !mask.any() {
            return self.background.eval_raw(rx, ry, rz, w);
        }

        // 4. Compute hit point P = ray * t
        let hx = rx * t;
        let hy = ry * t;
        let hz = rz * t;

        // 5. Optimization: Early exit if all hit (material only)
        if mask.all() {
            return self.material.eval_raw(hx, hy, hz, w);
        }

        // 6. Mixed case: compute both and select
