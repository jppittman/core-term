    ...
    B: ManifoldCompat<Jet3, Output = Field>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        // Derivatives are Fields, but arithmetic on them builds AST nodes.
        // We must evaluate the AST to get a Field result for comparison.
        // Since the expression is constant (composed of Fields), we can eval with unit domain.
        let deriv_mag_sq = (t.dx * t.dx + t.dy * t.dy + t.dz * t.dz).eval(());
        let valid_deriv = deriv_mag_sq.lt(deriv_max_sq);
        let mask = valid_t & valid_deriv;

        // Optimization: Conditional evaluation
        // Default to zero if we can't determine yet
        let mut result = zero;

        // If any pixel hits the object, evaluate material
        if mask.any() {
            // 3. Hit point: P = ray * t
            let hx = rx * t;
            let hy = ry * t;
            ...
            // If ALL pixels hit object, we can return material directly
            if mask.all() {
                return mat_val;
            }
            result = mat_val;
        ...
        // If any pixel misses (hits background), evaluate background
        if !mask.all() {
            ...
            // Mixed case: blend
            result = Selectable::select_raw(mask, result, bg_val);
        }
    ...
    B: ManifoldCompat<Jet3, Output = Discrete>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let valid_t = t_val.gt(zero) & t_val.lt(t_max);
        // Evaluate derivative magnitude AST
        let deriv_mag_sq = (t.dx * t.dx + t.dy * t.dy + t.dz * t.dz).eval(());
        let valid_deriv = deriv_mag_sq.lt(deriv_max_sq);
        let mask = valid_t & valid_deriv;

        // Optimization: Conditional evaluation
        // Use a dummy default (e.g. all zeros) - will be overwritten
        let mut result = Discrete::default();
        ...
        if !mask.all() {
            ...
            // Mixed case: blend
            result = Discrete::select(mask, result, bg_val);
        }
