...
kernel!(pub struct Surface = |geometry: kernel, material: kernel, background: kernel| Jet3 -> Field {
    ...
    let valid__t = (V(t) > 0.0) & (V(t) < t_max);
    let deriv_mag_sq = DX(t) * DX(t) + DY(t) * DY(t) + DZ(t) * DZ(t);
    let valid__deriv = deriv_mag_sq < (deriv_max * deriv_max);
    let hit_mask = valid_t.clone() & valid_deriv.clone();
    ...
    let mat_val = material.at(h_x, h_y, h_z, W);
