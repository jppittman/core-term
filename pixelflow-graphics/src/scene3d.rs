    ...
    B: ManifoldCompat<Jet3, Output = Field>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let valid_t = (t.val.gt(Field::from(0.0)) & t.val.lt(t_max)).eval(());

        let mut mat_val = Field::from(0.0);
        let mut mask = Field::from(0.0);

        if valid_t.any() {
            let deriv_mag_sq = t.dx * t.dx + t.dy * t.dy + t.dz * t.dz;
            let valid_deriv = deriv_mag_sq.lt(deriv_max * deriv_max).eval(());
            mask = (valid_t & valid_deriv).eval(());
    ...
    B: ManifoldCompat<Jet3, Output = Discrete>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let mut mat_val = <Discrete as Computational>::from_f32(0.0);
        let mut mask = Field::from(0.0);
        ...
        let mut bg_val = <Discrete as Computational>::from_f32(0.0);
        if !mask.all() {
            bg_val = self.background.eval_raw(rx, ry, rz, w);
        }
