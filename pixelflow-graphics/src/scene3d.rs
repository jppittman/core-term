...
/// Epsilon for sphere intersection discriminant check to avoid numerical noise.
pub const SPHERE_EPSILON: f32 = 0.0001;

/// Minimum cosine of incidence angle for reflection to prevent division by zero.
pub const REFLECT_MIN_COS_INCIDENCE: f32 = 0.1;

/// Scaling factor for curvature-aware reflection.
pub const CURVATURE_SCALE_FACTOR: f32 = 2.0;

/// Epsilon for vector normalization to avoid division by zero.
pub const NORMALIZATION_EPSILON: f32 = 1e-10;

/// Threshold for checkerboard pattern decision.
pub const CHECKER_THRESHOLD: f32 = 0.25;

/// Epsilon for antialiasing gradient magnitude to prevent division by zero.
pub const AA_EPSILON: f32 = 0.001;
...
impl<M: ManifoldCompat<Jet3, Output = Field>> Manifold<Jet3_4> for Reflect<M> {
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let inv_n_len = n_len_sq.max(Field::from(NORMALIZATION_EPSILON)).sqrt().rsqrt();
...
impl<C: ManifoldCompat<Field, Output = Discrete>> Manifold<Jet3_4> for ColorChecker<C> {
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let pixel_size = (grad_x.max(grad_z) + Field::from(AA_EPSILON)).constant();
