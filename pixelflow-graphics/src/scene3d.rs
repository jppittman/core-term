    ...
    B: ManifoldCompat<Jet3, Output = Field>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Field {
        ...
        let t = self.geometry.eval_raw(x, y, z, w);
        ...
        let deriv_mag_sq_expr = t.dx * t.dx + t.dy * t.dy + t.dz * t.dz;
        let deriv_mag_sq = deriv_mag_sq_expr.eval(p);
        let valid_deriv = deriv_mag_sq.lt(deriv_max_sq);
        let mask = valid_t & valid_deriv;

        // 3. Conditional evaluation to avoid redundant work
        let mut result = if !mask.all() {
            self.background.eval_raw(x, y, z, w)
        } else {
            Field::from(0.0) // Placeholder, fully overwritten by material
        };
        ...
        if mask.any() {
            ...
            if mask.all() {
                ...
                result = Field::select_raw(mask, mat_val, result);
            }
        }
    ...
    B: ManifoldCompat<Jet3, Output = Discrete>,
{
    ...
    fn eval(&self, p: Jet3_4) -> Discrete {
        ...
        let mut result = if !mask.all() {
            ...
            Discrete::from_f32(0.0) // Placeholder
        };
        ...
        if mask.any() {
            ...
            if mask.all() {
                ...
                result = Discrete::select(mask, mat_val, result);
            }
        }
