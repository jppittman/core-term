    ...
    fn eval(&self, p: Field4) -> Field {
        if self.is_linear {
            ...
            let k = kernel!(|ax: f32, bx: f32, cx: f32, by: f32, cy: f32| {
                let t0 = (Y - cy) / by;
                let in_t = t0.clone().ge(0.0) & t0.clone().le(1.0);

                // x-coordinate at intersection
                let x_int = t0.clone() * t0.clone() * ax + t0.clone() * bx + cx;
        ...
        let k = kernel!(|ax: f32, bx: f32, cx: f32, ay: f32, by: f32,
            ...
            let t__plus = sqrt_disc.clone() * inv_2a.clone() + neg_b_2a.clone();
            let t__minus = sqrt_disc * -inv_2a + neg_b_2a;

            // X-coordinates at intersection points
            let x_plus = t__plus.clone() * t__plus.clone() * ax.clone() + t__plus.clone() * bx.clone() + cx.clone();
            let x_minus = t__minus.clone() * t__minus.clone() * ax + t__minus.clone() * bx + cx;

            // Tangent dy/dt at each root for winding direction
            let dy_plus = t__plus.clone() * (2.0 * ay.clone()) + by.clone();
            let dy_minus = t__minus.clone() * (2.0 * ay) + by;
            ...
            let valid_plus = t__plus.clone().ge(0.0) & t__plus.clone().le(1.0);
            let valid_minus = t__minus.clone().ge(0.0) & t__minus.clone().le(1.0);
