...
mod tests {
    ...
    /// Helper to get a font for testing, skipping if LFS data is missing.
    fn get_test_font() -> Option<Font<'static>> {
        let font = Font::parse(FONT_DATA);
        if font.is_none() {
            eprintln!("Skipping font test: LFS asset data missing (pointer file found)");
        }
        font
    }
    ...
    fn test_cached_glyph_creation() {
        let Some(font) = get_test_font() else { return };
