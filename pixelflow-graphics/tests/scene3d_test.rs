...
/// Benchmark: Compare Optimization (Lazy) vs Baseline (Eager) Surface evaluation.
#[test]
fn test_optimization_benchmark() {
    const W: usize = 1920;
    const H: usize = 1080;

    // 1. Define Eager Surface (simulates old behavior)
    #[derive(Clone, Copy, ManifoldExpr)]
    pub struct ColorSurfaceEager<G, M, B> {
        pub geometry: G,
        pub material: M,
        pub background: B,
    }

    impl<G, M, B> Manifold<Jet3_4> for ColorSurfaceEager<G, M, B>
    where
        ...
        B: ManifoldCompat<Jet3, Output = Discrete>,
    {
        ...
        fn eval(&self, p: Jet3_4) -> Discrete {
            ...
            let valid_t = t_val.gt(Field::from(0.0)) & t_val.lt(t_max);

            let dx = t.dx;
            let dy = t.dy;
            let dz = t.dz;
            let deriv_mag_sq = (dx * dx + dy * dy + dz * dz).eval(p);
            let deriv_max = Field::from(10000.0);
            let valid_deriv = deriv_mag_sq.lt((deriv_max * deriv_max).eval(p)).eval(p);
            let mask = (valid_t & valid_deriv).eval(p);

            // EAGER EVALUATION (No early exit)
            let hx = rx * t;
            let hy = ry * t;
            let hz = rz * t;
    ...
    // 2. Setup Scene
    let world_mat = ColorChecker::<RgbaColorCube>::default();
    let world_bg = ColorSky::<RgbaColorCube>::default();
    let sphere_geo = SphereAt { center: (0.0, 0.0, 4.0), radius: 1.0 };
    let plane_geo = plane(-1.0);

    // World (Floor + Sky)
    let world_eager = ColorSurfaceEager {
        geometry: plane_geo,
        material: world_mat,
        background: world_bg,
    };
    let world_lazy = ColorSurface {
        geometry: plane_geo,
        material: world_mat,
        background: world_bg,
    };
    ...
    // Scene (Sphere + World)
    let scene_eager = ColorSurfaceEager {
        geometry: sphere_geo,
        material: ColorReflect { inner: world_eager.clone() },
        background: world_eager,
    };
    let scene_lazy = ColorSurface {
        geometry: sphere_geo,
        material: ColorReflect { inner: world_lazy.clone() },
        background: world_lazy,
    };
    ...
    impl<M: ManifoldCompat<Field, Output = Discrete>> Manifold<Field4> for ColorScreenRemap<M> {
        ...
        fn eval(&self, p: Field4) -> Discrete {
            ...
            let x = (px - width * Field::from(0.5)) * scale.clone();
            let y = (height * Field::from(0.5) - py) * scale;
            At { inner: &self.inner, x, y, z, w }.collapse()
        }
    }

    let render_eager = ColorScreenRemap {
        inner: ColorScreenToDir { inner: scene_eager },
        width: W as f32,
        height: H as f32,
    };

    let render_lazy = ColorScreenRemap {
        inner: ColorScreenToDir { inner: scene_lazy },
        width: W as f32,
        height: H as f32,
    };
    ...
    // Warmup
    rasterize(&render_lazy, &mut frame, 1);

    let start_eager = std::time::Instant::now();
    rasterize(&render_eager, &mut frame, 1);
    ...
    let mpps_eager = (W * H) as f64 / time_eager.as_secs_f64() / 1_000_000.0;
    let mpps_lazy = (W * H) as f64 / time_lazy.as_secs_f64() / 1_000_000.0;
    let speedup = mpps_lazy / mpps_eager;

    println!("BENCHMARK RESULTS (1920x1080):");
    println!("Eager Surface (Old): {:.2} ms ({:.2} Mpix/s)", time_eager.as_secs_f64() * 1000.0, mpps_eager);
    println!("Lazy Surface (New):  {:.2} ms ({:.2} Mpix/s)", time_lazy.as_secs_f64() * 1000.0, mpps_lazy);
    println!("Speedup:             {:.2}x", speedup);

    assert!(speedup > 1.1, "Optimization should provide at least 10% speedup");
}
