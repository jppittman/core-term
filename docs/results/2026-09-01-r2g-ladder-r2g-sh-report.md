...
Per-expression rows: `docs/results/2026-09-01-r2g-ladder-r2g-sh.jsonl` (95 expressions: classical 95). Skipped: none.
...
| (d) LinearReturnGuide[return-mse] @B [claim] | 0.890 / 0.908 / 0.928 (p90 0.945) | 95 / 0 / 0 | -9.67% / -7.22% / -3.30% (p90 1.1%) | 3.66% / 8.24% / 20.33% (p90 32.2%) | 1.030 / 17.347 / 70.146 (p90 84.025) (n=76) | 0.703 | 0.0998 | 3 |

Ladder diagnostics: linear < control on 25 expressions, equal on 2, linear > control on 68. control: quiesced before B on 0 / 95 (ended at 800.000 / 800.000 / 800.000 (p90 800.000) applications; burned-key share 0.000 / 0.000 / 0.000 (p90 0.000)); reaches the empirical best on 6; vs production cost better / equal / worse = 3 / 1 / 91. linear: quiesced before B on 0 / 95 (ended at 800.000 / 800.000 / 800.000 (p90 800.000) applications; burned-key share 0.000 / 0.000 / 0.000 (p90 0.000)); reaches the empirical best on 4; vs production cost better / equal / worse = 3 / 0 / 92. 

**Verdict (B=100):** (d) LinearReturnGuide[return-mse] median ratio vs unguided-at-B = **0.908** against the registered threshold ≤ 0.837 (Y = 16.3%) — the Y-clause **FAILS**; median gap vs unguided-at-4B = -7.22% against ≤ 24.2% — the 4B-approach clause **HOLDS**; (d) beats unguided-at-B at all (median ratio < 1.0, kill-gate view). Ladder: (c) control median ratio 0.903, regret 7.92% vs (d) regret 8.24% — (d) **does not beat** (c).

**Round-1b domain-shift statistic (S = classical):** M_B = 0.06. D_control = 0.3373 (m_control^S = 0.9028, m_control^DEV = 0.5655). D_linear = 0.3718 (m_linear^S = 0.9084, m_linear^DEV = 0.5366). D_control − D_linear = -0.0345 → **H_null**. §1.2 polynomial prediction: both arms D ≤ 0: FAILS; both arms D ≤ M_B: FAILS.
...
| (d) LinearReturnGuide[return-mse] @B [claim] | 0.887 / 0.902 / 0.920 (p90 0.937) | 95 / 0 / 0 | -8.97% / -5.75% / -1.10% (p90 1.5%) | 2.88% / 7.72% / 17.36% (p90 31.9%) | 1.116 / 3.562 / 11.947 (p90 29.101) (n=93) | 0.740 | 0.0766 | 4 |

Ladder diagnostics: linear < control on 2 expressions, equal on 16, linear > control on 77. control: quiesced before B on 1 / 95 (ended at 800.000 / 800.000 / 800.000 (p90 800.000) applications; burned-key share 0.000 / 0.000 / 0.000 (p90 0.000)); reaches the empirical best on 6; vs production cost better / equal / worse = 3 / 1 / 91. linear: quiesced before B on 1 / 95 (ended at 800.000 / 800.000 / 800.000 (p90 800.000) applications; burned-key share 0.000 / 0.000 / 0.000 (p90 0.000)); reaches the empirical best on 4; vs production cost better / equal / worse = 3 / 1 / 91. 

**Verdict (B=200):** (d) LinearReturnGuide[return-mse] median ratio vs unguided-at-B = **0.902** against the registered threshold ≤ 0.910 (Y = 9.0%) — the Y-clause **HOLDS**; median gap vs unguided-at-4B = -5.75% against ≤ 11.0% — the 4B-approach clause **HOLDS**; (d) beats unguided-at-B at all (median ratio < 1.0, kill-gate view). Ladder: (c) control median ratio 0.894, regret 6.70% vs (d) regret 7.72% — (d) **does not beat** (c).

**Round-1b domain-shift statistic (S = classical):** M_B = 0.07. D_control = 0.1949 (m_control^S = 0.8940, m_control^DEV = 0.6991). D_linear = 0.2058 (m_linear^S = 0.9017, m_linear^DEV = 0.6959). D_control − D_linear = -0.0109 → **H_null**. §1.2 polynomial prediction: both arms D ≤ 0: FAILS; both arms D ≤ M_B: FAILS.
...
| 20 | doubling | linear | 396 (0, 0.0%) | 883 (0, 0.0%) | 3533 (0, 0.0%) | 93 |
...
| 39 | half-angle-product | linear | 545 (134, 24.6%) | 745 (134, 18.0%) | 1147 (134, 11.7%) | 93 |
...
| linear | 798 / 996 (80.1%) | 798 / 996 (80.1%) |
...
| linear | 0.703 | 0.740 | reverse-associative 2583, associative 2077, fma-fusion 949, commutative 868, canonicalize 646, half-angle-product 545, doubling 396, halving 364 |
...
- load_at_end: 19:32  up 51 days, 10:05, 2 users, load averages: 5.80 6.66 8.78
- load_at_start: 19:32  up 51 days, 10:04, 2 users, load averages: 6.34 6.82 8.90
