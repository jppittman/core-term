...
|---|---|---|
| Cell-grid packed kernel: any geometry change (cols/rows/cell px/atlas extents) | 1 compile per distinct geometry; **every resize is a recompile** | [`core-term/src/terminal_app.rs:362-388`](../../core-term/src/terminal_app.rs#L362-L388) |
...
| `eval_scalar` oracle | **TEST-ONLY by design** (`oracle` feature under `[dev-dependencies]` everywhere) | [`pixelflow-ir/src/eval.rs:1-7`](../../pixelflow-ir/src/eval.rs#L1-L7); [`pixelflow-ir/Cargo.toml:29`](../../pixelflow-ir/Cargo.toml#L29); edges in compiler/codegen/graphics/search `Cargo.toml` are dev-deps; `pixelflow-pipeline` under `training` feature |
...
| Retired four-plane `CellGridProgram` in scene.rs | **DEAD in production** (test module only) | [`pixelflow-graphics/src/render/scene.rs:373-384,447`](../../pixelflow-graphics/src/render/scene.rs#L373-L384); core-term uses `CellGridPackedProgram` ([`terminal_app.rs:14,119,373`](../../core-term/src/terminal_app.rs#L14)) |
...
| §5 pre-registered experiment | Budget **denominated in rule applications** ("median ~195, heavy-tailed"), cost = `CostModel::latency_prior()`, "no wall-clock timing", family-held-out split ([`guide-design-revision.md:289-345`](../plans/2026-08-31-guide-design-revision.md#L289-L345)) | **MISMATCH in the budget's unit.** Production budgets rounds, classes, and milliseconds, selected by node count; it does not count or expose rule applications ([`graph.rs:881-884`](../../pixelflow-search/src/egraph/graph.rs#L881-L884)). The wall-clock ceiling that production has (10/50/200 ms) is excluded from the experiment by construction. The cost model matches. |
...
5. [`pixelflow-search/src/runtime.rs:74`](../../pixelflow-search/src/runtime.rs#L74) — if the knob is to be observable, the `SaturationResult` currently dropped at `:128` has to come back out (a `_with_stats` sibling or a log line), otherwise there is no way to see the knob bind on a real kernel.
