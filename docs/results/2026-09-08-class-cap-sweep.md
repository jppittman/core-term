# The class-cap sweep: the classical cap is sized by the input, and past that budget makes extraction worse

**Shipped:** the classical class cap is `clamp(8 × inserted_classes, 5,000, 50,000)` (`SaturationConfig::classical_for`, `pixelflow-search/src/egraph/saturate.rs`), with the application budget at the plan's 40 per class of the resolved cap and the safety ceiling at 1.5 ms per application (300 s at the floor, as before). 44 of the 95 DejaVu glyphs are raised (to 5,448–31,952 classes); every other DEV kernel and chrome insert to at most 465 classes and keep the flat 5,000 — byte-identical. Against the flat cap the raised glyphs extract **−16.7% Σ `dag_cost`**, −31% guarded entries, 1,074 → 20 spill slots, −0.3% bytes, none dearer; Σ glyph-warm compile per 95 glyphs goes **6.6 s → 25.5 s (+290%)**, which exceeds the corrected benchmark's +5% bound by ~60× and is stated here rather than hidden. `docs/plans/2026-09-01-production-budget-determinism.md` carries the revision.
...
`SaturationConfig::classical_for(inserted)`; `Budget::Production` resolves through `config_for_input(InputSize { nodes, classes })`; `Optimizer::run` reads the inserted count off the e-graph on entry (the input has been inserted and nothing rewritten). Shipped rows (`rows/ship`, production path, asserted byte-identical to `Manifold::compile`), against the same binary's production rows before the change:
