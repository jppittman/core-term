...
| `LevelSectionedEdges` — the per-level edge accumulator as a feature *type*, no consumer today | `pixelflow-search/src/nnue/factored.rs` (restored under #1093 (b), renamed so it cannot be mistaken for a value head) | the seed of the by-level inputs of §2.2. **What was actually restored (state the shape honestly):** the old `EdgeAccumulator` verbatim under the new name — four sections that are flat/depth-encoded × parent/child edge sums over the *whole* DAG, plus a four-bin variance histogram (`variance_histogram(arena)`: const / frame / scanline / pixel node fractions), `from_arena_dag` / `from_extraction(.., with_variance)`, and the `add_edge*` / `remove_edge*` / `remove_var_ref_edges` ops kept per A6 of the domain doc (the O(Δ) justification is gone; harmless). The name is aspirational: it is level-*aware*, not yet level-*sectioned*. The change that makes it the §2.2 input — sections **indexed by level** (`const, frame, scanline, pixel`, extensible to binders), each holding its own edge sums, with the classification taken from the *chosen* nodes' variance (P1(c)) rather than a histogram over the arena — is step 2 of §5.1, done when a level is an e-node attribute, not before |
