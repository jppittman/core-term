...
| **per-level sectioned edge accumulator** — a feature *type*, no consumer today | restored under #1093 (b), named so it cannot be mistaken for a value head, e.g. `LevelSectionedEdges` | the by-level inputs of §2.2. **Shape note for the restore:** the deleted `EdgeAccumulator` was *not* sectioned by variance class — its four sections were flat/depth-encoded × parent/child, and variance was four scalar node fractions. The per-level type is sections **indexed by level** (`const, frame, scanline, pixel`, extensible to binders) each holding the edge sums; what can be revived verbatim is the `CostDag::variance()` hook, `Extraction::chosen_variance` (variance of the chosen nodes, P1(c)), the classification rule, and the removal ops if A6's "keep the API" ruling stands (the O(Δ) justification is gone; harmless) |
| `Extraction` + swap search behind `Reranker` | `pixelflow-search/src/egraph/extract.rs` | the reranking primitive: `try_swap`'s cycle-checked candidate construction and the best-improvement loop, generic over `Reranker`, with the analytic reranker as the only implementation and the `extract_dag`-identity test as its contract |
...
1. #1077 (one kernel ABI — merged 2026-09-01) → #1080 stages 0–2 (`LoopShape`
