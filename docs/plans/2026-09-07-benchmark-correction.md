...
- `docs/results/2026-09-07-claims-ledger.md` (+ `.csv`, 92 rows; PR #1207, branch `claude/claims-ledger` until it merges) — every quantitative claim, its corpus, units, instrument and verdict.
...
- **First three re-validation items:** 0 the CI gate on deterministic columns; 1 finish F and take its clock at load < 8 with the < 10% rules-beyond-CSE rule (decided by the cse-vs-on clock, not on-vs-off); 2 rule order on real kernels in DAG units with the ≤ 0.97-and-no-family-worse rule.
