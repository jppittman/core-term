...
* **Terminal Emulator (Interpreter/VM):** The core state machine (`struct TerminalEmulator`). Maintains the terminal grid, cursor, attributes, modes, selection state, etc. Interprets `EmulatorInput` to modify state (including internal dirty flags) and signals `EmulatorAction`s. **Does not perform I/O or rendering directly.**
...
* **Rendering Driver (`Driver` Trait):** Interface implemented by backend-specific components (`XDriver`, `ConsoleDriver`, and planned `WaylandDriver`, `MacosDriver`). Responsibilities:
    ...
    * **Translating platform-specific input events into abstract `BackendEvent`s.** This includes converting mouse pixel coordinates to cell coordinates.
    * **Executing a list of abstract `RenderCommand`s,** translating them into platform-specific API calls.
    * Provides necessary info like event FD, font metrics (cell pixel dimensions), display scale factor, and current display dimensions.
...
* **`main.rs`:**
    * Entry point, initial setup (including loading `Config`).
...
* **`orchestrator.rs` (`AppOrchestrator` struct):**
    ...
    * **Driver Event Handling:** Polls `Driver` -> `driver.process_events()` (which returns `Vec<BackendEvent>`).
        * Handles `BackendEvent::Resize`: Calls `driver.get_font_dimensions()`, `driver.get_scale_factor()`, calculates new grid dimensions, informs `pty.resize()`, `term.update_dimensions()`, and potentially `term.update_cell_pixel_size()`.
        * Handles `BackendEvent::CloseRequested` by signaling shutdown.
        * Handles `BackendEvent::Key`:
            * Checks against configured hotkeys (e.g., for copy/paste from `Config.keybindings`). If a hotkey matches, creates appropriate `EmulatorInput::User(UserInputAction::InitiateCopy)` or `EmulatorInput::User(UserInputAction::InitiatePaste)`.
...
* **`term` Module (`TerminalEmulator` struct, `screen.rs`, `cursor.rs`, `selection.rs` etc.):**
    ...
    * Holds all terminal state (grid, cursor, attributes, modes, selection state, internal dirty flags for each line, cell pixel dimensions for SGR pixel reporting). **No built-in scrollback.**
...
* **`backends` Module (`Driver` trait, `BackendEvent` enum, `RenderCommand` enum, `x11.rs`, `console.rs`, future `wayland.rs`, `macos.rs`):**
    * Defines `trait Driver` with methods like `process_events() -> Result<Vec<BackendEvent>>`, `get_font_dimensions()`, `get_scale_factor()`, `get_dimensions_cells`, and crucially `execute_commands(commands: Vec<RenderCommand>) -> Result<()`.
        * `get_font_dimensions()` returns cell pixel dimensions, accounting for scale factor.
        * `get_scale_factor()` returns the display scale factor (e.g., 1.0, 2.0).
    * Defines `enum BackendEvent` with platform-agnostic variants.
        * `Key { symbol: KeySymbol, modifiers: Modifiers, text: Option<String> }`.
        * `Mouse { event_type: MouseEventType, col: usize, row: usize, button: MouseButton, modifiers: Modifiers }` (col/row are 0-based cell coordinates).
    * Defines `enum RenderCommand` (the Lingua Franca for rendering).
    * Concrete `Driver` implementations:
        * Translate platform-specific events (including mouse pixel coordinates) to abstract `BackendEvent`s. May use a shared utility function like `pixels_to_cells(pixel_x, pixel_y, cell_width_px, cell_height_px)` for coordinate conversion.
...
* **`config` Module (`config.rs`):**
    * Defines `Config` struct, including `KeybindingsConfig` with `KeyCombination { symbol: KeySymbol, modifiers: Modifiers }` for actions like copy and paste.
...
2.  **User Input (Key):** `Driver` detects platform-specific key event -> Translates to abstract `BackendEvent::Key` -> `AppOrchestrator` calls `driver.process_events()` -> `AppOrchestrator` receives `BackendEvent::Key`.
    * If hotkey (e.g., copy/paste): `AppOrchestrator` calls `term.interpret_input(EmulatorInput::User(UserInputAction::InitiateCopy/Paste))` -> Handles `EmulatorAction` (e.g., `CopyToClipboard`, `RequestClipboardContent`). For paste, gets clipboard content, then `term.interpret_input(EmulatorInput::User(UserInputAction::PasteText(...)))`.
...
4.  **User Input (Resize):** (Largely same as v1.8, with added `get_scale_factor()` and `update_cell_pixel_size()` calls).
...
* **Platform Support (Wayland, macOS):**
    ...
    * The `Driver` trait will include `fn get_scale_factor(&self) -> f64;` for HiDPI support.
    * macOS event loop integration will likely use a pipe-based mechanism to signal the `AppOrchestrator`'s main event loop.
* **Configuration:** A dedicated mechanism for user configuration (partially addressed with `KeybindingsConfig`).
...
* **`BackendEvent` Definition (Abstract Platform Input):**
    ```rust
    // Example structure, to be defined in `backends` module.
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)] // For KeySymbol, Modifiers, MouseButton, MouseEventType
    pub enum KeySymbol { /* As previously defined */ }
    bitflags! { pub struct Modifiers: u8 { /* As previously defined */ } }
