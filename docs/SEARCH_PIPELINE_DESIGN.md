...
### Pipeline 3: Unified Mask Search (Bilinear Scoring)

```rust
let pipeline = SearchPipeline {
    proposer: UnifiedMaskProposer::new(
        model: ExprNnue::load("judge.bin"),
        templates: rule_templates,
        threshold: 0.3,
    ),
    evaluator: CostEvaluator::new(costs),
    trainer: ReinforceTrainer::new(
        // REINFORCE: trajectory reward from final extraction cost
    ),
};
```
...
1. **Batch Embedding Strategy**: Should expr embeddings be batch-computed at epoch start or lazily on demand?
   - Pro batch: Amortizes backbone forward pass, simple ownership
   - Pro lazy: Lower memory, only computes for visited e-classes
