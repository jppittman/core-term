...
// ── Checkpoint: mint-style provenance (schema identity, strict-v1 label source) ──
...
/// `pixelflow_pipeline::training::mint::MintMetadata`'s pattern (content-hash
/// schema identity, a named label/objective source, a weights content hash)
/// without reusing that type directly — `MintMetadata` is shaped around
/// bench-timing provenance (`bench_mode`, `sentinel_calibration_ns`,
/// `normalization`) the Guide has none of, since every number in this
/// program is a deterministic `CostModel::latency_prior()` count, never a
/// timing (design doc §0's binding framing). `label_source` is the field
...
struct GuideCheckpoint {
    schema_identity: String,
    label_source: String,
    trainer: String,
    written_at_unix_s: u64,
    ...
    num_rules: usize,
    num_ops: usize,
    /// `rule_idx -> name`, `""` for an index never observed in TRAIN or DEV
    /// — kept dense (indexable by `rule_idx` directly) so a future loader
    /// need not re-derive it from `all_rules()`.
    ...
    /// checkpoint files.
    weights_fnv64: String,
}

impl SchemaIdentity for GuideCheckpoint {
    ...
    const SCHEMA: &'static str = "\
        ...
        op_names/w_op; \
        rule_names: rule_idx -> human name, dense, \"\" for an unseen index; \
        ...
        linear model's weights — logit = bias + w_rule[rule_idx] + \
...
impl GuideCheckpoint {
    #[must_use]
    fn current_schema_identity() -> String {
    ...
    fn weights_fingerprint(&self) -> String {
    ...
    /// Read and validate a checkpoint written by [`Self::write`]. Not called
    /// by `main` (this binary only writes), but exercised by this module's
    /// round-trip test — the loud-refusal contract every `SchemaIdentity`
    /// consumer must implement per that trait's doc, so the seam exists
    /// before the first real consumer (a future Guide-wiring binary) needs
    /// it, rather than being invented ad hoc at that point.
    #[allow(dead_code)]
...
fn main() {
    ...
    let train = load_jsonl(&PathBuf::from(&args.train), &op_index);
    ...
    for (s, &p) in dev.samples.iter().zip(dev_scores.iter()) {
        let e = dev_mean_pred_by_rule.entry(s.rule_idx).or_insert((0.0, 0));
        e.0 += p as f64;
        e.1 += 1;
    }
...
mod tests {
    ...
    fn checkpoint_write_then_read_round_trips_and_validates() {
        ...
        let mut ckpt = GuideCheckpoint {
            ...
            num_rules: 2,
            num_ops: 1,
            rule_names: vec!["a".to_string(), "b".to_string()],
            op_names: vec!["Add".to_string()],
            bias: 0.1,
            w_rule: vec![0.2, -0.3],
            w_op: vec![0.4],
            w_budget: 0.5,
    ...
    #[test]
    fn checkpoint_read_refuses_a_hand_edited_weight_with_a_stale_hash() {
        let dir = std::env::temp_dir().join(format!(
            "train_guide_ckpt_tamper_test_{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("checkpoint.json");
