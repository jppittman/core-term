...
fn main() {
    ...
    for (i, (name, arena, root)) in entries.iter().enumerate() {
        ...
        let total_applications = egraph.provenance().application_count() as u64;
    ...
    let mut rule_rows: Vec<(usize, RuleAgg)> = rule_agg
        .iter()
    ...
    let class_pooled = |names: &[&str]| -> RuleAgg {
        ...
        for (idx, a) in &rule_agg {
            if names.contains(&name_of(*idx).as_str()) {
                agg.fired += a.fired;
    ...
    if let Some(out_path) = &args.out {
        ...
        for (i, (idx, agg)) in rule_rows.iter().enumerate() {
            json.push_str(&format!(
                "    {{\"rule\": \"{}\", \"rule_idx\": {idx}, \"fired\": {}, \"loose_load_bearing\": {}, \"tight_load_bearing\": {}, \"strict_load_bearing\": {}}}{}\n",
                name_of(*idx),
                agg.fired,
                agg.loose_lb,
                agg.tight_lb,
