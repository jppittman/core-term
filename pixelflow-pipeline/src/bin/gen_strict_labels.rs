...
/// JSON output — the key's own fields (`rule_idx` + the private
...
fn mint_split(
    ...
    for (i, (name, arena, root)) in entries.iter().enumerate() {
        ...
        for (app_id, record) in egraph.provenance().applications() {
            ...
            let agg = stats.rule_agg.entry(record.rule_idx).or_default();
            agg.fired += 1;
            if positive {
                agg.positive += 1;
            }
        ...
        assert_eq!(
            total_applications,
            egraph.provenance().application_count(),
...
fn print_split_report(stats: &SplitStats, rule_names: &[String]) {
    println!(
    ...
    let mut rows: Vec<(usize, RuleAgg)> = stats.rule_agg.iter().map(|(&k, &v)| (k, v)).collect();
    rows.sort_by(|a, b| {
...
fn write_json_report(path: &str, train: &SplitStats, dev: &SplitStats, rule_names: &[String]) {
    ...
    let write_split = |json: &mut String, s: &SplitStats| {
        ...
        let mut rows: Vec<(usize, RuleAgg)> = s.rule_agg.iter().map(|(&k, &v)| (k, v)).collect();
...
fn main() {
    ...
    print_split_report(&train_stats, &rule_names);
    print_split_report(&dev_stats, &rule_names);
    ...
    if let Some(report_path) = &args.report_json {
        write_json_report(report_path, &train_stats, &dev_stats, &rule_names);
    }
}
