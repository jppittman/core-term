...
fn compare_expression(name: &str, expr: ExprTree, judge: &DualHeadNnue) {
    ...
    let (old_tree, old_additive_cost) = egraph_old.extract_best(root_old, &cost_model);
    let old_neural_cost = predict_tree_cost(&old_tree, judge);

    println!("  [2] OLD OPTIMIZER (CostModel + additive extraction)");
    ...
    println!("      Additive cost: {} (CostModel units)", old_additive_cost);
