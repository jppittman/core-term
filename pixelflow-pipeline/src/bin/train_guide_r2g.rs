...
use pixelflow_pipeline::training::guide_linear::{Model, Sample, op_index_table, to_sample};
use pixelflow_pipeline::training::r2g::{
    LabelBudget, R2gRecord, RegressionTarget, sample_rank_pairs, target_value,
};
...
fn load_jsonl(
    ...
    for (line_no, line) in reader.lines().enumerate() {
        ...
        samples.push(to_sample(&record.base, op_index));
...
impl SchemaIdentity for GuideCheckpointR2g {
    ...
    const SCHEMA: &'static str = "\
        ...
        model's weights — logit = bias + w_rule[rule_idx] + sum_op(hist[op]*w_op[op]) + \
...
impl GuideCheckpointR2g {
    ...
    fn write(&mut self, path: &PathBuf) -> std::io::Result<()> {
...
fn write_md_report(path: &str, report: &Report) {
    ...
    md.push_str("| rule | idx | DEV fired | mean predicted return | mean realized return |\n");
    md.push_str("|---|---:|---:|---:|---:|\n");
...
fn main() {
    ...
    for i in 0..dev.records.len() {
        ...
        let rule_idx = dev.records[i].base.rule_idx;
        let z = model.logit(&dev.samples[i]);
        let e = agg.entry(rule_idx).or_insert((0.0, 0.0, 0));
        e.0 += z as f64;
        e.1 += y as f64;
        e.2 += 1;
    }
    let mut per_rule_rows: Vec<RuleRow> = agg
        .into_iter()
        .map(|(rule_idx, (pred_sum, actual_sum, n))| RuleRow {
            rule_idx,
            rule_name: rule_names[rule_idx].clone(),
            dev_fired: n,
    ...
    let mut checkpoint = GuideCheckpointR2g {
...
mod tests {
    ...
    #[test]
    fn checkpoint_write_then_read_round_trips_and_carries_the_objective_field() {
        let dir = std::env::temp_dir().join(format!("train_guide_r2g_ckpt_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("checkpoint.json");

        let mut ckpt = GuideCheckpointR2g {
            schema_identity: String::new(),
            label_family: "return-to-go".to_string(),
        ...
        let loaded = GuideCheckpointR2g::read(&path).unwrap();
        assert_eq!(loaded.objective, "return-mse");
        assert_eq!(loaded.label_family, "return-to-go");
        assert_eq!(loaded.w_rule, vec![0.2, -0.3]);

        // And the raw JSON on disk carries the top-level "objective" key —
        // the exact field pixelflow_search::nnue::guide::linear::LinearReturnGuide
        // and LinearCandidateGuide gate their cross-loading refusal on.
        let raw: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        assert_eq!(
            raw.get("objective").and_then(|v| v.as_str()),
            Some("return-mse")
        );
