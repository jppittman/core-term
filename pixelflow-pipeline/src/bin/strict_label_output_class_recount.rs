...
use pixelflow_search::egraph::{
    ...
    ENodeId, EpisodeLabels, Origin, all_rules, config_for_node_count, run_anytime_curve,
};
...
fn chosen_path(out: &AnytimeCurveOutput) -> HashMap<EClassId, ENodeId> {
    ...
    let mut stack = vec![out.extraction.root];
...
fn rule_name(eg: &EGraph, idx: usize) -> String {
    eg.rule(idx)
        .unwrap_or_else(|| panic!("rule index {idx} not in the e-graph's rule list"))
        .name()
        .to_string()
}

fn origin_label(eg: &EGraph, tag: ENodeId) -> String {
...
fn tally_expression(
    ...
    by_rule_idx: &mut BTreeMap<usize, RuleTally>,
    ...
    let mut fired_here: BTreeSet<usize> = BTreeSet::new();
    ...
    for (app, rec) in prov.applications() {
        ...
        let rname = rule_name(eg, rec.rule_idx);
        let t = by_rule_idx.entry(rec.rule_idx).or_default();
        t.name = rname.clone();
        t.fired += 1;
        match effect {
        ...
        let chosen_origin = chosen_here.map(|tag| origin_label(eg, tag));
        if let Some(o) = &chosen_origin {
            *t.chosen_origin_when_class_on_path
                .entry(o.clone())
                .or_default() += 1;
        }
        fired_here.insert(rec.rule_idx);
        ...
        if rname == "pythagorean" {
            let row = pyth.get_or_insert_with(|| PythagoreanRow {
                ...
                extracted: render_chosen(eg, &out.extraction.choices, out.extraction.root),
    ...
    if let Some(row) = pyth.as_mut() {
        for app in &labels.strict {
            ...
            *row.strict_credited_rules
                .entry(rule_name(eg, rec.rule_idx))
                .or_default() += 1;
        }
    }
    pyth
...
fn main() {
    ...
    let mut by_rule_idx: BTreeMap<usize, RuleTally> = BTreeMap::new();
    ...
    for (i, (name, arena, root)) in selected.iter().enumerate() {
        ...
        let apps = out.egraph.provenance().application_count();
        total_applications += apps;
        let row = tally_expression(name, arena, &out, &mut by_rule_idx);
        let pyth_note = row
            .as_ref()
            .map(|r| {
                format!(
