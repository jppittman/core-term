...
use super::backprop::{forward_with_state_hybrid, backward_hybrid};
...
pub struct ExprGenerator {
    ...
    max_depth: usize,
    num_vars: usize,
    seed_ops: Vec<pixelflow_ir::OpKind>,
}
...
impl ExprGenerator {
    pub fn new(seed: u64) -> Self {
        ...
        assert!(!seed_ops.is_empty(), "No seed ops found in OpKind");
    ...
    fn generate_inner(&mut self, depth: usize) -> ExprTree {
        ...
        if depth >= self.max_depth || self.rng.gen_f64() < leaf_prob {
            ...
            let kind = self.seed_ops[self.rng.gen_range(0..n)];
            let op = op_from_kind(kind).unwrap_or_else(|| {
                panic!("OpKind::{} marked is_seed_op but has no Op struct", kind.name())
            });
            let children: Vec<ExprTree> = (0..kind.arity())
                .map(|_| self.generate_inner(depth + 1))
                .collect();
...
pub fn run_judge_training(config: BenchmarkConfig, cache_path: &Path) {
    ...
    let mut train_indices: Vec<usize> = train_families.iter()
        ...
        .collect();
    let test_indices: Vec<usize> = test_families.iter()
        .flat_map(|f| families.get(f).unwrap().iter().copied())
        .collect();
