...
/// - `fusion_rules()` from pixelflow-pipeline (FMA, rsqrt, mul_rsqrt)
///
...
fn main() {
    ...
    let bench_path = workspace_root.join("pixelflow-pipeline/benches/generated_kernels.rs");
    ...
    // Generate FAST benchmark file (no criterion overhead)
    let fast_bench_path = workspace_root.join("pixelflow-pipeline/src/bin/fast_kernels.rs");
    let fast_code = codegen::generate_fast_benchmark_file(&pending_benchmarks);
    let mut fast_file = File::create(&fast_bench_path).expect("Failed to create fast benchmark file");
    fast_file
        .write_all(fast_code.as_bytes())
        .expect("Failed to write fast benchmark");
    println!("Wrote {} fast benchmarks to {}", pending_benchmarks.len(), fast_bench_path.display());

    // Also generate criterion benchmark (slower but more precise)
    ...
    println!("\nNext steps (FAST - recommended):");
    println!("  1. Run fast bench: cargo run -p pixelflow-pipeline --bin fast_kernels --release");
    println!("  2. Merge data:     (timing data written to pixelflow-pipeline/data/fast_timings.jsonl)");
    println!("  3. Retrain Judge:  cargo run -p pixelflow-pipeline --bin train_judge --release --features training");
    println!("\nAlternative (SLOW but more precise):");
    println!("  cargo bench -p pixelflow-pipeline --bench generated_kernels");
    println!("  cargo run -p pixelflow-pipeline --bin collect_judge_data --release");
