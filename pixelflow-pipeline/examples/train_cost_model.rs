...
impl TrainableCostModel {
    ...
    pub fn predict(&self, expr: &Expr) -> f32 {
        match expr {
            Expr::Var(_) | Expr::Const(_) | Expr::Param(_) => 0.0,
            Expr::Unary(op, a) => {
            ...
            Expr::Nary(op, _children) => {
                panic!("TrainableCostModel::predict: Nary({}) not supported", op.name());
            }
        }
    }
    ...
    fn count_ops_inner(&self, expr: &Expr, counts: &mut OpCounts) {
        match expr {
            ...
            Expr::Nary(op, _children) => {
                panic!("count_ops_inner: Nary({}) not supported", op.name());
            }
        }
    }
}
...
impl OpCounts {
    pub fn to_vec(&self) -> Vec<f32> {
        vec![
            ...
            self.min as f32, self.max as f32, self.mul_add as f32,
        ]
    }
}
...
use pixelflow_pipeline::nnue::{Expr, OpType, BwdGenerator, BwdGenConfig, RuleTemplates};
...
fn expr_hash_inner<H: std::hash::Hasher>(expr: &Expr, hasher: &mut H) {
    ...
    match expr {
        ...
        Expr::Param(i) => { 5u8.hash(hasher); i.hash(hasher); }
        Expr::Nary(op, children) => {
            6u8.hash(hasher);
            (*op as u8).hash(hasher);
            for child in children {
                expr_hash_inner(child, hasher);
            }
        }
    }
}
...
pub fn train_cost_model(config: &TrainConfig) -> TrainableCostModel {
    ...
    let mut generator = BwdGenerator::new(42, BwdGenConfig::default(), templates);
...
pub fn evaluate_model(model: &TrainableCostModel, test_size: usize) -> f32 {
    let templates = collect_rule_templates();
    let mut generator = BwdGenerator::new(9999, BwdGenConfig::default(), templates);
    let mut correct = 0;
