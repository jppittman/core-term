...
//!
//! Entry point for NNUE training. Delegates to `pixelflow_pipeline::training::egraph`.
//!
//! ## Usage
//!
//! ```bash
...
use pixelflow_pipeline::training::egraph::{
...
use pixelflow_pipeline::training::features::{extract_search_features, extract_tree_features};
use pixelflow_pipeline::training::backprop::{forward_with_state, forward_with_state_hybrid};
use pixelflow_pipeline::nnue::{Nnue, NnueConfig};
    ...
    for epoch in 0..config.kindergarten_epochs {
        ...
        for _ in 0..config.kindergarten_samples {
            ...
            let result = planner.run(|ctx: BestFirstContext<'_>| {
                ...
                let (pred, _) = pixelflow_pipeline::training::backprop::forward_with_state(evaluator, &features);
                (pred * 1000.0) as i64
            });
            ...
            let features = pixelflow_pipeline::training::features::extract_tree_features(&result.best_tree);
...
fn trace_forward_pass(judge: &Nnue, sample_idx: usize, sample: &pixelflow_pipeline::training::egraph::JudgeTrainingSample) {
    ...
    for i in 0..pixelflow_pipeline::nnue::DenseFeatures::COUNT {
...
fn compute_linear_baseline(samples: &[pixelflow_pipeline::training::egraph::JudgeTrainingSample]) -> f64 {
    use pixelflow_pipeline::nnue::DenseFeatures;
...
fn evaluate_judge(judge: &Nnue, workspace: &PathBuf) {
    ...
    let compute_metrics = |samples: &[pixelflow_pipeline::training::egraph::JudgeTrainingSample], label: &str| {
        let mut actuals = Vec::new();
        let mut predictions = Vec::new();
