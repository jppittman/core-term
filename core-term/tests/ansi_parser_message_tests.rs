...
const HIGH_THROUGHPUT_CMD_CAPACITY: usize = 1000;
const SCHEDULER_QUEUE_SIZE: usize = 10;
const HIGH_THROUGHPUT_QUEUE_SIZE: usize = 100;
const SCHEDULER_BUFFER_SIZE: usize = 64;
const HIGH_THROUGHPUT_BUFFER_SIZE: usize = 128;
const DEFAULT_SLEEP_MS: u64 = 50;
const SHORT_SLEEP_MS: u64 = 10;
const LONG_SLEEP_MS: u64 = 200;
const E2E_SLEEP_MS: u64 = 100;
...
fn cuj_pty02_real_parser_simple_text() {
    ...
    let (cmd_tx, cmd_rx) = sync_channel::<Vec<AnsiCommand>>(CMD_CHANNEL_CAPACITY);
    let bytes_processed = Arc::new(AtomicUsize::new(0));

    let (parser_tx, mut parser_rx) =
        ActorScheduler::<Vec<u8>, (), ()>::new(SCHEDULER_QUEUE_SIZE, SCHEDULER_BUFFER_SIZE);
...
fn cuj_pty02_real_parser_incremental_escape() {
    ...
    thread::sleep(Duration::from_millis(SHORT_SLEEP_MS));
...
fn cuj_pty02_real_parser_high_throughput() {
    // Given: A real parser actor
    let (cmd_tx, cmd_rx) = sync_channel::<Vec<AnsiCommand>>(HIGH_THROUGHPUT_CMD_CAPACITY);
    let bytes_processed = Arc::new(AtomicUsize::new(0));

    let (parser_tx, mut parser_rx) = ActorScheduler::<Vec<u8>, (), ()>::new(
        HIGH_THROUGHPUT_QUEUE_SIZE,
    ...
    thread::sleep(Duration::from_millis(LONG_SLEEP_MS));
    drop(parser_tx);
    parser_handle.join().unwrap();
...
fn cuj_e2e_complete_message_chain() {
    ...
    thread::sleep(Duration::from_millis(E2E_SLEEP_MS));
    chain.shutdown();
