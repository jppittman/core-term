...
pub(super) fn process_ansi_command(
    ...
    match command {
        AnsiCommand::C0Control(c0) => handle_c0(emulator, c0),
        AnsiCommand::Esc(esc_cmd) => handle_esc(emulator, esc_cmd),
        AnsiCommand::Csi(csi) => handle_csi(emulator, csi),
...
fn handle_c0(emulator: &mut TerminalEmulator, c0: C0Control) -> Option<EmulatorAction> {
    match c0 {
        C0Control::BS => {
            emulator.backspace();
            None
...
fn handle_esc(emulator: &mut TerminalEmulator, esc_cmd: EscCommand) -> Option<EmulatorAction> {
    match esc_cmd {
...
fn handle_csi(emulator: &mut TerminalEmulator, csi: CsiCommand) -> Option<EmulatorAction> {
    match csi {
