...
fn it_should_scroll_up_entire_screen_by_1_line_on_csi_s_with_param_0_or_1() {
    ...
    let initial_screen_line0_char0 =
        get_glyph_from_snapshot(&term.get_render_snapshot().expect("Snapshot was None"), 0, 0)
            .unwrap()
            .display_char();
...
fn it_should_set_indexed_foreground_color_sgr_38_5_n() {
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('A')));
    let attr =
        match get_glyph_from_snapshot(&term.get_render_snapshot().expect("Snapshot was None"), 0, 0)
            .unwrap()
        {
            Glyph::Single(c) => c.attr,
            _ => panic!("Expected Single"),
