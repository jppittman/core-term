...
fn it_should_print_a_single_ascii_character() {
    ...
    let snapshot = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_wrap_character_to_next_line_when_end_of_line_is_reached() {
    ...
    let snapshot_before_wrap = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_after_wrap = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_print_ascii_over_wide_char_that_straddles_line_end_after_wrap() {
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('A')));
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('世')));
    let s1 = term.get_test_snapshot().expect("Snapshot was None");
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('X')));
    let s2 = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let s3 = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let s4 = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_move_cursor_down_keeping_column_on_line_feed_if_lnm_is_off() {
    ...
    let snapshot_before_lf = term.get_test_snapshot().expect("Snapshot was None");
    assert_screen_state(
        &snapshot_before_lf,
        &["A   B     ", "          ", "          "],
        Some((0, 5)),
    ...
    let snapshot_after_lf = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_final = term.get_test_snapshot().expect("Snapshot was None");
    assert_screen_state(
        &snapshot_final,
        &["A   B     ", "     C    ", "          "],
        Some((1, 6)),
...
fn it_should_scroll_up_and_move_cursor_down_keeping_column_on_line_feed_at_bottom_if_lnm_is_off() {
    ...
    let snapshot_before_scroll = term.get_test_snapshot().expect("Snapshot was None");
    assert_screen_state(&snapshot_before_scroll, &["123  ", "ABC  "], Some((1, 3)));

    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::C0Control(C0Control::LF)));
    let snapshot_after_scroll = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_move_cursor_to_col_0_on_carriage_return() {
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('X')));
    let snapshot2 = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_move_cursor_to_next_tab_stop_on_horizontal_tab() {
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::C0Control(C0Control::HT)));
    let snapshot3 = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_move_cursor_to_last_column_on_horizontal_tab_if_no_more_tab_stops() {
    ...
    let snapshot_before = term.get_test_snapshot().expect("Snapshot was None");
    assert_screen_state(&snapshot_before, &["012345678 "], Some((0, 9)));

    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::C0Control(C0Control::HT)));
    let snapshot_after = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_erase_from_cursor_to_end_of_screen_on_csi_0j_or_csi_j() {
    ...
    let snapshot0 = term_clone.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_default = term.get_test_snapshot().expect("Snapshot was None");
    assert_screen_state(
        &snapshot_default,
        &["ABCAB", "BC   ", "     "],
        Some((1, 2)),
...
fn it_should_scroll_up_entire_screen_by_1_line_on_csi_s_with_param_0_or_1() {
    ...
    assert_screen_state(
        &term_clone.get_test_snapshot().expect("Snapshot was None"),
        &["BCDBC", "CDECD", "     "],
        Some((1, 2)),
    );
...
fn it_should_handle_resize_with_content_and_cursor_at_edges() {
    ...
    let snapshot_larger = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_medium = term.get_test_snapshot().expect("Snapshot was None");
    ...
    // Cursor (1,2)
    term.interpret_input(EmulatorInput::Control(resize_event(2, 1)));
    let snapshot_smaller = term.get_test_snapshot().expect("Snapshot was None");
...
fn it_should_show_and_hide_cursor_on_dectcem() {
    ...
    let snapshot_hidden = term.get_test_snapshot().expect("Snapshot was None");
    assert!(
    ...
    let snapshot_shown = term.get_test_snapshot().expect("Snapshot was None");
    assert!(
...
fn it_should_switch_to_alternate_screen_buffer_and_back_on_csi_1049() {
    ...
    let original_snapshot = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_asb = term.get_test_snapshot().expect("Snapshot was None");
    ...
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('X')));
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('Y')));
    let snapshot_asb_content = term.get_test_snapshot().expect("Snapshot was None");
    ...
    let snapshot_nsb_restored = term.get_test_snapshot().expect("Snapshot was None");
