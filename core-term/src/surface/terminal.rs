...
use pixelflow_render::{BakedMask, Memoize};
use std::sync::Arc;

pub type GlyphCache =
    Memoize<char, Arc<BakedMask>, Arc<dyn Fn(char) -> Arc<BakedMask> + Send + Sync>>;
...
mod tests {
    ...
    use pixelflow_render::{font, glyphs_scaled};

    fn create_cache() -> GlyphCache {
        let f = font();
        Memoize::new(Arc::new(move |c| {
            // Mock bake
            let g = glyphs_scaled(f, 16.0)(c);
            Arc::new(BakedMask::new(&g, 10, 16))
        }))
    }
