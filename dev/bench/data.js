    ...
    "Benchmark": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "58885c5bbd7884e6747a91b74bcb34bcb7956b10",
          "message": "test: close mutation-test gaps and STYLE.md violations (2026-08-05 audit) (#981)\n\n## Summary\n\nThis is a scheduled test-quality-control pass, scoped to code touched\nsince the last audit (PR #971, 2026-08-01) — mainly the `pixelflow-ir` →\n`pixelflow-codegen` split (#974).\n\n- **Mutation testing** (`cargo mutants -p pixelflow-codegen -f\npixelflow-codegen/src/jit_cache.rs`) found 4 surviving mutants:\n- `entry_count()`'s body could be replaced with a hardcoded `0` or `1`\nand no test noticed.\n- A `+` → `-`/`*` slicing bug in `canonical_key`'s N-ary-child range\n(`s..s+l`) went undetected because no test exercised a `Reduce` (N-ary)\nnode at all.\n\nAdded tests that exercise `entry_count()` and back-to-back `Reduce`\nnodes through the public `compile_cached` API. Re-running mutants\nconfirms all 4 are now caught (11 caught / 3 unviable, up from 7 caught\n/ 4 missed / 3 unviable).\n\n- **STYLE.md \"Test Public API\" audit** of everything changed since the\nlast audit found 4 violations (tests calling private functions directly\ninstead of exercising the public contract):\n- `pixelflow-codegen/src/jit_cache.rs`: a test called the private\n`canonical_key`/`Mode` directly — rewritten to go through\n`compile_cached` + `Arc::ptr_eq`, same as its sibling tests (fixed\nalongside the mutation-test work above, since it's the same\nfile/function).\n- `pixelflow-pipeline/src/training/corpus.rs`: three error-path tests\n(`bad_magic_fails`, `bad_version_fails`,\n`v1_corpus_is_refused_with_regeneration_hint`) called the private\n`read_corpus_bytes` — now they write fixture bytes to a temp file and go\nthrough the public `read_corpus`.\n- `pixelflow-pipeline/src/training/unified_backward.rs`: a `prob ==\nsigmoid(score)` assertion called the private `sigmoid()` helper — now\ncomputed independently (`1.0 / (1.0 + libm::expf(-score))`) so the test\nchecks the documented contract, not the helper's own implementation.\n- `pixelflow-codegen/src/emit/mod.rs`: three tests\n(`arena_to_schedule_simple`, `arena_to_schedule_filters_unreachable`,\n`verify_arena_to_uses`) call the private\n`arena_to_schedule`/`arena_to_uses` directly with no public equivalent\navailable. Per STYLE.md's \"Break Rules Sensibly\" clause, these check\nschedule-shape invariants (value numbering, dead-node filtering) that\nhave no output-value signature — a regression here changes\nregister/instruction efficiency, not what a compiled kernel computes —\nso no public black-box assertion could catch a break here. Added a\ncomment documenting why the rule is deliberately broken, matching the\nexisting `surviving_dwrt_fails_loudly` test in the same file.\n\n## Test plan\n\n- [x] `cargo test -p pixelflow-codegen -p pixelflow-pipeline --release`\n— all passing\n- [x] `cargo clippy -p pixelflow-codegen -p pixelflow-pipeline --tests\n--release` — clean\n- [x] `cargo mutants -p pixelflow-codegen -f\npixelflow-codegen/src/jit_cache.rs` — 11 caught / 3 unviable / 0 missed\n(was 7/3/4)\n\n---\n_Generated by [Claude\nCode](https://claude.ai/code/session_01LaFC47XnJg8LG3A9RokCv4)_\n\nCo-authored-by: Claude <noreply@anthropic.com>",
          "timestamp": "2026-08-08T16:13:51-07:00",
          "tree_id": "cd6153fa5743841739751dc08634c6104335d672",
          "url": "https://github.com/jppittman/pixelflow/commit/58885c5bbd7884e6747a91b74bcb34bcb7956b10"
        },
        "date": 1786234360147,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "79201594f24295e86cd2a3ba411ca3d1401ec2fd",
          "message": "test(actor-scheduler): GreenSender::send timeout coverage (2026-07-29 audit) (#964)\n\n## Summary\n\nScheduled test-quality-control pass\n(`docs/bugs/2026-07-29-test-quality-audit-followup.md`), continuing the\nseries in `docs/bugs/`. Covers the 28 commits since the last pass\n(`d7d33b8..HEAD`).\n\n- **Static audit**: swept all 19 test files changed in this range\nagainst `docs/STYLE.md`'s \"Test Public API\" rule and the\ncomments/historical-narration rule. No violations found — the changes\nare rustfmt reflow, import reorders, or new tests built entirely from\npublic API.\n- **Mutation testing** (`cargo-mutants`) on the three genuinely new\npieces of logic in this diff range — nothing a prior pass had touched:\n- `pixelflow-core/src/lattice/cell_grid.rs` (new file): 6 missed mutants\n→ 0. Real gaps: a degenerate `rows=1` test fixture hid a\nmultiply-vs-divide bug in `cells_len`, coincidentally-equal fg/bg\nfixture values hid a per-channel buffer-index bug, the y-axis apron\nclamp had no test (only x was covered), and no test straddled the\nsum-vs-product grid-extent boundary.\n- `actor-scheduler/src/host.rs`: 1 missed → 0. `GreenSender::send` (new\nbackoff-based delivery path) had zero direct test coverage in this\ncrate.\n- `pixelflow-runtime/src/coordinator_node.rs` (new file): 2 missed → 0.\n`CoordinatorData`'s `Debug` impl was never exercised, and\n`present_cooked_frame`'s frame-number increment was checked for presence\nbut never for its actual value.\n\n## Test plan\n\n- [x] `cargo test -p pixelflow-core --lib`: 112/112\n- [x] `cargo test -p actor-scheduler --lib`: 138/138\n- [x] `cargo test -p pixelflow-runtime --lib`: 64/64\n- [x] `cargo check --workspace`: clean\n- [x] `cargo fmt --check` on all touched crates: clean\n- [x] Re-ran `cargo-mutants` on all three files after the fixes to\nconfirm each gap is actually closed (numbers in the doc), not just that\nthe new tests pass\n\n🤖 Generated with [Claude Code](https://claude.ai/code)\n\nhttps://claude.ai/code/session_019AmfMxL8Rh2VF9uwZ7JF5e\n\n---\n_Generated by [Claude\nCode](https://claude.ai/code/session_019AmfMxL8Rh2VF9uwZ7JF5e)_\n\nCo-authored-by: Claude <noreply@anthropic.com>",
          "timestamp": "2026-08-09T08:44:55-07:00",
          "tree_id": "4b47adfa739c8f1e7a0fa1ffd428422a43a81a88",
          "url": "https://github.com/jppittman/pixelflow/commit/79201594f24295e86cd2a3ba411ca3d1401ec2fd"
        },
        "date": 1786293742771,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "d049a07ef1f83bc9715ffa3717ca574e4cbf30b9",
          "message": "feat(pixelflow): the terminal screen as ONE packed JIT kernel (3.05x steady-state) (#980)\n\nRebased onto main (post #972/#974/#976/#979). Eight commits, reviewed\nbottom-up.\n\n## What this is\nThe cell grid — the terminal's production frame path — rewritten in the\nkernel language and compiled as **one packed program**: samplers\ncomposed at computed coordinates, four channels sharing\ngeometry/cell-reads/atlas-taps through e-graph CSE, packing to `u32`\npixels in-register through the type-blind collapse store. Byte order\nlives on the ColorCube family (`PlatformColorCube` picks the shifts\nexactly as it picks the cube; a test pins them to `Pixel::from_rgba`);\napplications pass geometry and colors only.\n\n## Steady-state numbers (2560×1584, single thread, median of 15,\nre-measured after rebase)\n| path | ns/px | frame |\n|---|---|---|\n| four-plane + per-pixel pack (optimized, main's compile entry) | 28.0 |\n114 ms |\n| **packed kernel + row copy** | **10.9** | **44 ms** |\n\n**2.58×** over the four-plane path (3.05× over what shipped before\nmain's optimizer-in-compile-entry landed). Also deletes 1 MiB/worker of\nplane scratch and the per-pixel pack loop. The packed kernel extracts\n623 → 121 nodes — cross-channel geometry, cell reads, and atlas taps\nmerged.\n\n## Load-bearing substrate (bottom of the stack)\n- **`BufferIdentity`**: process-unique identity on `BufferDecl`;\n`splice` merges buffer tables by it, so buffer-bearing kernels compose\n(previously: panic) and N reads of one buffer bind one slot. The\noptimizer cache keys buffers by identity, not slot index — two arenas'\n\"slot 0\" can name different memory.\n- **E-graph totality over the runtime IR**: `Buffer` as an `ENode` leaf\ncarrying its decl; `Gather` + the integer/bitwise domain as opaque\nruntime-tier ops — CSE-only, unnameable by rewrite templates,\nunreachable by the folder. Without this the packed kernel bailed at the\ndoor and benchmarked 1.01× against four planes (both unoptimized — the\ntie is what exposed it). Pinned by `packed_pixel_shape_optimizes`.\n- **Cell grid in the language**: `channel_arena` (60 hand-pushed IR\nnodes × 4) deleted; `DiscreteManifold`/`BilinearSampler` expose\ncomposable Kernel fragments keyed on extents (data binds later).\n- **CocoaWaker main-thread dispatch** (the intermittent AppKit launch\ncrash) + a CI job that launches the real bundle repeatedly, which `cargo\ntest` can never exercise.\n- Profiling substrate for the squeeze work: `JitManifold::code_bytes` +\nin-process dump/hot-loop harnesses. First profile: **100.0% of frame\nself-time is inside the JIT kernel** — the row copy does not register.\n\nDropped during rebase as superseded by main: the `from_index` UB fix\n(dense renumbering dissolved the gaps) and the cost-table match\nconversion (#976's `OpMap::from_fn` is the same shape).\n\n🤖 Generated with [Claude Code](https://claude.com/claude-code)\n\n---------\n\nCo-authored-by: JP Pittman <jppittman@jpptech.dev>\nCo-authored-by: Claude Opus 5 <noreply@anthropic.com>",
          "timestamp": "2026-08-09T16:42:01Z",
          "tree_id": "e08a7c39b394bff2742e5d53cfb02ce986fe83d4",
          "url": "https://github.com/jppittman/pixelflow/commit/d049a07ef1f83bc9715ffa3717ca574e4cbf30b9"
        },
        "date": 1786297297546,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "876f5e0baea7a8a2672ea9ed9bd7ff555d6c5fbb",
          "message": "test(actor-scheduler): fix sharded.rs/host.rs mutation gaps, mealy.rs public-API violations (#969)\n\n## Summary\n\nScheduled test-quality-control pass (continuation of the recurring audit\nseries in `docs/bugs/2026-07-{20,24,26,28}-test-quality-audit*.md`).\nFull writeup: `docs/bugs/2026-07-31-test-quality-audit-followup.md`.\n\n- **`mealy.rs`**: three new `step_os` tests read the private\n`Node.continuation` field directly instead of relying on public return\nvalues (a new STYLE.md \"Test Public API\" violation, caught by a static\naudit of everything changed since the last pass). Rewrote all three to\nprove the same facts through `poll`/`poll_os`'s public `(Step,\nActorStatus)` return values instead.\n- **`sharded.rs`**: fixed 12 of 18 missed `cargo-mutants` mutants in\n`ShardedInbox::drain`/`take_one`'s round-robin arithmetic — a gap first\nflagged 2026-07-20 and never closed in four follow-up passes since.\n`take_one` (reached via the public `Inbox::take` trait method) had\n**zero** test coverage at all before this. The remaining 6 misses were\nhand-verified as genuine equivalent mutants *before* the fix, then\nconfirmed empirically by re-running cargo-mutants after (documented in\nthe audit doc).\n- **`host.rs`**: `GreenSender::send` (the public, documented\nbackoff+wake send path) and its `PortTarget::try_deliver` impl (how a\ngreen-to-green pipeline stage delivers) were never exercised by any\nexisting test — every prior test used the `pub(crate)` `try_send`\ninstead, so a mutant replacing either body with `Ok(())` went\nundetected. Added coverage for both.\n\n## Test plan\n\n- [x] `cargo test -p actor-scheduler --lib`: 144/144 passing (was 130\nbefore this pass's 14 new tests)\n- [x] `cargo test -p actor-scheduler` (incl. integration tests and\ndoctests): all passing\n- [x] `cargo check --workspace`: clean\n- [x] `cargo clippy -p actor-scheduler --lib --tests`: clean\n- [x] `cargo mutants` re-run scoped to `sharded.rs`: 18 missed → 6 (all\nconfirmed equivalent)\n- [x] `cargo mutants` re-run scoped to `host.rs`: 2 missed → 0\n\n---\n_Generated by [Claude\nCode](https://claude.ai/code/session_01AvLuW9PtWsaGwossAtqfww)_\n\nCo-authored-by: Claude <noreply@anthropic.com>",
          "timestamp": "2026-08-09T10:30:57-07:00",
          "tree_id": "be9bd0a344d2cf0995b02d93e42f9364ccf6bfa0",
          "url": "https://github.com/jppittman/pixelflow/commit/876f5e0baea7a8a2672ea9ed9bd7ff555d6c5fbb"
        },
        "date": 1786300679960,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "507753a8b3446d3ea9e77964d53d865c2dff55bd",
          "message": "fix(fold): guard out-of-range shifts; correct the comment that hid them (#989)\n\nFollow-up to the `TruncToInt` fold fix in #980. A sweep for other\nfolder-vs-emitter divergences found a second and worse instance, plus\nthe stale comment that let both survive.\n\n## Shift counts >= 32 have three answers\n\n| | answer |\n|---|---|\n| folder | `x.to_bits() << (y as u32 & 31)` — reduces the count **mod\n32** |\n| x86 | `(V)PSLLD`/`(V)PSRLD` zero the **entire destination** for any\ncount > 31 (Intel SDM) |\n| aarch64 | `emit_shl` computes `immh:immb = shift + 32`, so at shift >=\n32 the carry into `immh` makes it `1xxx` — which decodes as **`.2D`**, a\n64-bit element shift that leaks bits **across the 32-bit lane boundary**\n|\n\nMeasured on `aarch64-apple-darwin` by executing the emitted words:\n`shift=33` emits `SHL .2D #1`, and input lanes `[0x80000000, 0x00000001,\n...]` came back `[0x00000000, 0x00000003, ...]` — lane 1 is 3, not 2,\nbecause bit 31 of lane 0 crossed into it.\n\nSo this is not only a fold divergence. **The emitter produces a silently\ndifferent instruction whether or not a constant is involved** — a\nnon-const `Shl` with an out-of-range count is already wrong today, and\nthe two architectures are wrong in different ways.\n\n`fold_is_platform_specific` now refuses a count outside `0..32` or a\nnon-integral one, and both aarch64 shift emitters assert their encodable\nrange instead of quietly changing element size. `Bits::shl` already\nasserted `bits < 32`, but the raw `arena.push_binary(OpKind::Shl, ..)`\npath used by `passes.rs` did not — the type-level refusal existed on\nexactly one of the two entry points.\n\n## The comment that hid it\n\n`runtime.rs` asserted the runtime-only integer ops were unreachable by\nfolding:\n\n> and `ConstantFold` cannot reach them because folding is rule-driven.\n\nThat is false. `ConstantFold::apply` destructures any `ENode::Op` and\nreads `op.kind()`; it never consults `op_from_kind`, so being unnameable\nby a template stops nothing. Every op registered there folds, and a\nmiscompile lived in the gap the claim created. Verified empirically\nthrough `optimize_runtime_arena` — `shl(0xFF, 32)` folded to `0xFF` —\nbefore rewriting the comment to say what actually holds.\n\nThis is the CLAUDE.md thesis in miniature (*a convention written in a\ncomment is an invariant something else will eventually break*), so the\nrepair is a guard in the type's own classifier plus an assert at the\nencoder, not a stronger comment.\n\n## Also: minimize the `TruncToInt` predicate from #980\n\nAn exhaustive 2^32 scan showed the shipped guard was **sound** — zero\nunsound admits — but refused **813,694,976** values on which all three\ntiers agree: the whole `x < -2^31` half, including `-inf`. There x86's\ninteger-indefinite pattern *is* `i32::MIN`, which is also what\nsaturation produces, so nothing diverges. The predicate is now\n`x.is_nan() || x >= 2^31`, verified bit-for-bit against models of all\nthree tiers over every f32.\n\nThe comment claiming \"the two architectures disagree with each other\"\nwas true of the worked example (`+inf`) and false of half the set it was\napplied to. Corrected, along with the CLAUDE.md row (which also had a\n`combinator tier` column entry for an op that has no combinator form).\n\n## Verification\n\n- `cargo test --workspace` — **1819 passed, 0 failed**\n- `cargo clippy --workspace --all-targets --all-features -- -D warnings`\n— exit 0\n- `cargo fmt --all --check` — clean\n- New tests: `out_of_range_shift_counts_are_platform_specific`,\n`aarch64_shl_refuses_a_lane_crossing_shift`, and the `TruncToInt` test\nextended to pin that the negative-overflow half **still folds**\n\nThe new emitter asserts fire on no existing lowering —\n`expand_exp2`/`log2` shift by 23, well inside range.\n\n## Not in this PR\n\nThe sweep confirmed two further divergences that change optimizer\nsemantics rather than merely refusing bad input, so they want your call\nrather than mine — details in the PR discussion.\n\n🤖 Generated with [Claude Code](https://claude.com/claude-code)\n\n---------\n\nCo-authored-by: JP Pittman <jppittman@jpptech.dev>\nCo-authored-by: Claude Opus 5 <noreply@anthropic.com>",
          "timestamp": "2026-08-11T04:20:33Z",
          "tree_id": "a943db813c930e5d080fce64cd4187c04ba6f688",
          "url": "https://github.com/jppittman/pixelflow/commit/507753a8b3446d3ea9e77964d53d865c2dff55bd"
        },
        "date": 1786425586628,
        "tool": "cargo",
        "benches": [
