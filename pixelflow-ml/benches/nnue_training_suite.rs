...
fn sub(a: Expr, b: Expr) -> Expr {
    Expr::Binary(OpType::Sub, Box::new(a), Box::new(b))
}
...
fn neg(a: Expr) -> Expr {
    Expr::Unary(OpType::Neg, Box::new(a))
}
fn abs(a: Expr) -> Expr {
    Expr::Unary(OpType::Abs, Box::new(a))
}
fn min(a: Expr, b: Expr) -> Expr {
    Expr::Binary(OpType::Min, Box::new(a), Box::new(b))
}
fn max(a: Expr, b: Expr) -> Expr {
    Expr::Binary(OpType::Max, Box::new(a), Box::new(b))
}
...
fn expensive_op_kernels() -> Vec<KernelPair> {
    vec![
        ...
        KernelPair::new(
            "sqrt3_deep",
            sqrt(add(sqrt(add(sqrt(x()), cnst(1.0))), cnst(1.0))),
        ),
...
fn distance_kernels() -> Vec<KernelPair> {
    vec![
        ...
        KernelPair::new(
            "dist3d_sq",
            add(add(mul(x(), x()), mul(y(), y())), mul(z(), z())),
        ),
        ...
        KernelPair::new(
            "box2d_sdf",
            max(sub(abs(x()), cnst(1.0)), sub(abs(y()), cnst(1.0))),
        ),
        // Normalize: x / sqrt(x² + y²)
        KernelPair::new(
            "normalize_x",
            div(x(), sqrt(add(mul(x(), x()), mul(y(), y())))),
        ),
    ]
}
...
fn polynomial_kernels() -> Vec<KernelPair> {
    vec![
        ...
        KernelPair::new(
            "quadratic",
            add(
                add(mul(cnst(2.0), mul(x(), x())), mul(cnst(3.0), x())),
                cnst(1.0),
            ),
        ),
        // Cubic: x³
        ...
        KernelPair::new(
            "cubic2v",
            add(mul(mul(x(), x()), x()), mul(mul(y(), y()), y())),
        ),
        // Cross term: x*y
...
fn depth_vs_width_kernels() -> Vec<KernelPair> {
    vec![
        ...
        KernelPair::new(
            "d4w1",
            add(
                add(add(add(x(), cnst(1.0)), cnst(2.0)), cnst(3.0)),
                cnst(4.0),
            ),
        ),
    ...
    vec![
        ...
        KernelPair::new(
            ...
            min(
                ...
                sub(
                    sqrt(add(
                        mul(sub(x(), cnst(3.0)), sub(x(), cnst(3.0))),
                        mul(y(), y()),
                    )),
                    cnst(1.0),
                ),
...
fn bench_expr_interpreter(c: &mut Criterion) {
    ...
    for kp in &kernels {
        ...
        println!(
            "  {}: HCE={}, crit_path={}, nodes={}, halfep_features={}",
            kp.name,
            kp.hce_cost,
            kp.critical_path,
            kp.expr.node_count(),
    ...
    for kp in &kernels {
        group.bench_with_input(BenchmarkId::new("expr", kp.name), &kp.expr, |b, expr| {
