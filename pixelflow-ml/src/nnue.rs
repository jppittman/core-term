...
impl Accumulator {
    ...
    pub fn forward(&self, nnue: &Nnue) -> i32 {
        ...
        for (i, &val) in self.values.iter().enumerate().take(l1_size) {
            ...
            for (j, &w2) in nnue.w2[i * l2_size..(i + 1) * l2_size].iter().enumerate() {
                l2[j] += (a as i32) * (w2 as i32);
            }
        }
        ...
        for (i, &val) in l2.iter().enumerate().take(l2_size) {
            let a = (val >> 6).clamp(0, 127) as i8;
            for (j, &w3) in nnue.w3[i * l3_size..(i + 1) * l3_size].iter().enumerate() {
                l3[j] += (a as i32) * (w3 as i32);
            }
        }
