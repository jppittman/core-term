...
// `path` encodes the position in the expression tree; it is intentionally
// only forwarded during recursion to track structural coordinates.
...
impl Accumulator {
    ...
    pub fn forward(&self, nnue: &Nnue) -> i32 {
        ...
        for (i, l2_val) in l2.iter().enumerate() {
            let a = (*l2_val >> 6).clamp(0, 127) as i8;
            for (j, l3_val) in l3.iter_mut().enumerate() {
        ...
        for (l3_val, w) in l3.iter().zip(nnue.w_out.iter()) {
