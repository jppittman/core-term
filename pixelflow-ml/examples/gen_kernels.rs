...
fn generate_backward(count: usize, seed: u64, bench_path: &PathBuf, data_dir: &PathBuf) {
    ...
    // Generate benchmark file with BOTH unoptimized and optimized expressions
    // This allows measuring actual SIMD cost savings
    let bench_kernels: Vec<(String, Expr)> = pairs
        .iter()
        .flat_map(|(name, unopt, opt)| {
            vec![
                (name.clone(), unopt.clone()),               // Unoptimized: bwd0000
                (format!("{}_opt", name), opt.clone()),      // Optimized: bwd0000_opt
            ]
        })
        .collect();
    ...
    let mut bench_file = File::create(bench_path).expect("Failed to create benchmark file");
    bench_file.write_all(bench_code.as_bytes()).expect("Failed to write benchmark");
    println!("Wrote benchmark to {} ({} kernels: {} unopt + {} opt)",
        bench_path.display(), bench_kernels.len(), pairs.len(), pairs.len());
...
/// Generate the complete benchmark file using kernel! macros.
///
/// Each expression is wrapped in kernel! so that:
/// - With `no_optimize` feature: benchmarks the EXACT expression given
/// - Without `no_optimize`: benchmarks the optimizer's rewritten version
///
/// This lets us measure the actual cost of specific expression structures
/// and train the cost model to predict which rewrites are beneficial.
fn generate_benchmark_file(kernels: &[(String, Expr)]) -> String {
    let mut code = String::new();

    // Header - note we use kernel! macro, not direct manifold ops
    code.push_str(r#"//! Auto-generated kernels for NNUE cost model training.
//!
//! Generated by: cargo run -p pixelflow-ml --example gen_kernels --features training
//!
//! ## Usage for cost model training:
//!
//! ```bash
//! # Benchmark with no_optimize to get true expression costs
//! cargo bench -p pixelflow-ml --bench generated_kernels --features no_optimize
//!
//! # Compare to optimized costs (optional)
//! cargo bench -p pixelflow-ml --bench generated_kernels
//! ```
//!
//! The difference reveals which rewrites actually improve performance,
//! which trains the cost model weights (MulAdd cost, ILP effects, etc.)
//!
...
"#);
    ...
    for (name, expr) in kernels {
        let kernel_body = expr_to_kernel_body(expr);

        code.push_str(&format!(r#"    // {name} - {nodes} nodes, depth {depth}
    {{
        let k = kernel!(|| {body});
        let m = k();
...
"#, name=name, nodes=expr.node_count(), depth=expr.depth(), body=kernel_body));
    }
...
/// Convert an Expr to kernel! body syntax.
/// This generates code that the kernel! macro will parse.
fn expr_to_kernel_body(expr: &Expr) -> String {
    match expr {
        Expr::Var(0) => "X".to_string(),
        Expr::Var(1) => "Y".to_string(),
        Expr::Var(2) => "Z".to_string(),
        ...
        Expr::Unary(op, a) => {
            let a_code = expr_to_kernel_body(a);
            match op {
                OpType::Neg => format!("(-{})", a_code),
                OpType::Sqrt => format!("({}).sqrt()", a_code),
                OpType::Rsqrt => format!("({}).rsqrt()", a_code),
                OpType::Abs => format!("({}).abs()", a_code),
                _ => panic!("Unexpected unary op: {:?}", op),
            }
        }
        ...
        Expr::Binary(op, a, b) => {
            let a_code = expr_to_kernel_body(a);
            let b_code = expr_to_kernel_body(b);
            match op {
                OpType::Add => format!("({} + {})", a_code, b_code),
                OpType::Sub => format!("({} - {})", a_code, b_code),
                OpType::Mul => format!("({} * {})", a_code, b_code),
                OpType::Div => format!("({} / {})", a_code, b_code),
                OpType::Min => format!("({}).min({})", a_code, b_code),
                OpType::Max => format!("({}).max({})", a_code, b_code),
                OpType::MulRsqrt => format!("({} * ({}).rsqrt())", a_code, b_code),
                _ => panic!("Unexpected binary op: {:?}", op),
            }
        }
        ...
        Expr::Ternary(op, a, b, c) => {
            let a_code = expr_to_kernel_body(a);
            let b_code = expr_to_kernel_body(b);
            let c_code = expr_to_kernel_body(c);
            match op {
                OpType::MulAdd => format!("({}).mul_add({}, {})", a_code, b_code, c_code),
                _ => panic!("Unexpected ternary op: {:?}", op),
            }
        }
    }
