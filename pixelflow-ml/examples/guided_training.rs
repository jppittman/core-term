...
fn evaluate_judge(judge: &Nnue, workspace: &PathBuf) {
    ...
    let median_actual = sorted_actuals
        .get(sorted_actuals.len() / 2)
        .copied()
        .unwrap_or(1.0);

    let mut sorted_preds = predictions.clone();
    sorted_preds.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let median_pred = sorted_preds
        .get(sorted_preds.len() / 2)
        .copied()
        .unwrap_or(1.0);

    let scale = if median_pred != 0.0 {
        median_actual / median_pred
    } else {
        1.0
    };
    ...
    println!(
        "    Spearman ρ   = {:.4}  (1.0 = perfect ranking)",
        spearman
    );
