...
use pixelflow_ml::nonlinear_eval::{linear_cost, interaction_cost, total_cost, critical_path_cost, ranking_correlation};
use pixelflow_ml::nnue::{ExprGenerator, ExprGenConfig, Expr, OpType};
...
fn main() {
    ...
    let hce_no_ilp = |e: &Expr| -> i32 {
        ...
        let mut score = 0i32;
        for i in 0..19 {  // Only first 19 features
    ...
    println!("  HCE+ILP vs HCE (no ILP):   {:.3}", hce_with_vs_without_ilp);
    println!();
    ...
    println!("Pairs where critical_path disagrees with total: {}/{} ({:.1}%)",
        different_rankings, total_pairs,
    ...
    let wide = Expr::Binary(
        OpType::Add,
        Box::new(Expr::Binary(OpType::Add, Box::new(Expr::Var(0)), Box::new(Expr::Var(1)))),
        Box::new(Expr::Binary(OpType::Add, Box::new(Expr::Var(2)), Box::new(Expr::Var(3)))),
    );
    // Deep: ((a+b)+c)+d
    ...
    println!("\nConclusion: Wide is {} by HCE (shorter critical path wins)",
        if hce.evaluate_linear(&wide_f) < hce.evaluate_linear(&deep_f) { "PREFERRED" } else { "NOT preferred" });
}
