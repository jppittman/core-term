...
use pixelflow_search::egraph::{EGraph, ExprTree, CostModel, codegen};
...
fn main() {
    ...
    for seed_idx in 0..count {
        ...
        let root = insert_tree(&mut eg, &seed_tree);
...
fn nnue_expr_to_tree(expr: &Expr) -> ExprTree {
    match expr {
        ...
        Expr::Const(v) => ExprTree::Const(*v),
        Expr::Unary(op, a) => {
            let a_tree = Box::new(nnue_expr_to_tree(a));
            match op {
                OpType::Neg => ExprTree::Neg(a_tree),
                OpType::Sqrt => ExprTree::Sqrt(a_tree),
                OpType::Rsqrt => ExprTree::Rsqrt(a_tree),
        ...
        Expr::Binary(op, a, b) => {
            let a_tree = Box::new(nnue_expr_to_tree(a));
            let b_tree = Box::new(nnue_expr_to_tree(b));
            match op {
        ...
        Expr::Ternary(op, a, b, c) => {
            let a_tree = Box::new(nnue_expr_to_tree(a));
            let b_tree = Box::new(nnue_expr_to_tree(b));
            let c_tree = Box::new(nnue_expr_to_tree(c));
            match op {
                OpType::MulAdd => ExprTree::MulAdd(a_tree, b_tree, c_tree),
                _ => ExprTree::Var(0), // Fallback
            }
        }
    }
...
fn tree_to_nnue_expr(tree: &ExprTree) -> Expr {
    match tree {
        ...
        ExprTree::Const(v) => Expr::Const(*v),
        ExprTree::Neg(a) => Expr::Unary(OpType::Neg, Box::new(tree_to_nnue_expr(a))),
        ExprTree::Sqrt(a) => Expr::Unary(OpType::Sqrt, Box::new(tree_to_nnue_expr(a))),
        ExprTree::Rsqrt(a) => Expr::Unary(OpType::Rsqrt, Box::new(tree_to_nnue_expr(a))),
        ExprTree::Abs(a) => Expr::Unary(OpType::Abs, Box::new(tree_to_nnue_expr(a))),
        ExprTree::Add(a, b) => Expr::Binary(
            OpType::Add,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::Sub(a, b) => Expr::Binary(
            OpType::Sub,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::Mul(a, b) => Expr::Binary(
            OpType::Mul,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::Div(a, b) => Expr::Binary(
            OpType::Div,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::Min(a, b) => Expr::Binary(
            OpType::Min,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::Max(a, b) => Expr::Binary(
            OpType::Max,
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
        ),
        ExprTree::MulAdd(a, b, c) => Expr::Ternary(
            ...
            Box::new(tree_to_nnue_expr(a)),
            Box::new(tree_to_nnue_expr(b)),
            Box::new(tree_to_nnue_expr(c)),
        ),
        // For other ops, approximate with simpler forms
        ExprTree::Recip(a) => Expr::Binary(
            OpType::Div,
        ...
        | ExprTree::Log2(a)
        | ExprTree::Log10(a) => {
            // These transcendentals don't have direct NNUE OpType equivalents
            // Use Sqrt as a proxy for "expensive unary op"
            Expr::Unary(OpType::Sqrt, Box::new(tree_to_nnue_expr(a)))
        }
        ExprTree::Atan2(a, b)
        | ExprTree::Pow(a, b)
        | ExprTree::Hypot(a, b)
        ...
        | ExprTree::Eq(a, b)
        | ExprTree::Ne(a, b) => {
            // Approximate with Div (expensive binary)
            Expr::Binary(
                OpType::Div,
                Box::new(tree_to_nnue_expr(a)),
                Box::new(tree_to_nnue_expr(b)),
        ...
        ExprTree::Select(cond, then_val, _else_val) => {
            // Approximate: use the then branch for features
            Expr::Binary(
                OpType::Mul,
                Box::new(tree_to_nnue_expr(cond)),
                Box::new(tree_to_nnue_expr(then_val)),
            )
        }
        ExprTree::Clamp(val, min, max) => Expr::Ternary(
            OpType::MulAdd,
            Box::new(tree_to_nnue_expr(val)),
            Box::new(tree_to_nnue_expr(min)),
            Box::new(tree_to_nnue_expr(max)),
        ),
        ExprTree::Tuple(elems) => {
            // Just use first element
            if let Some(first) = elems.first() {
                tree_to_nnue_expr(first)
            } else {
                Expr::Const(0.0)
            }
        }
