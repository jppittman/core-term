...
fn main() {
    ...
    let training_data: Vec<(Expr, f32)> = vec![
        ...
        (add(add(sqrt(x()), sqrt(y())), add(sqrt(z()), sqrt(w()))), 5.16),  // sqrt4_wide
    ...
    // Create samples
    let samples: Vec<NnueSample> = training_data.iter()
    ...
    println!("\n--- After Training ---");
    println!("Spearman correlation: {:.4} (was {:.4})", corr_after, corr_before);
    ...
    for (i, ((expr, cost), pred)) in training_data.iter().zip(preds_after.iter()).enumerate() {
        println!("  {:2}. pred={:6.3}, actual={:5.2}ns, nodes={}",
            i, pred, cost, expr.node_count());
    }
    ...
    println!("Actual: deep={:.2}ns, wide={:.2}ns (deep is FASTER)",
        training_data[sqrt_div_deep_idx].1,
        training_data[sqrt_div_wide_idx].1);
    println!("After:  deep={:.3}, wide={:.3} ({})",
        preds_after[sqrt_div_deep_idx],
        preds_after[sqrt_div_wide_idx],
        if preds_after[sqrt_div_deep_idx] < preds_after[sqrt_div_wide_idx] { "CORRECT ✓" } else { "WRONG ✗" });

    // Summary
    println!("\n=== Summary ===");
    if corr_after > 0.7 {
        println!("SUCCESS: NNUE learned to predict costs (Spearman = {:.3})", corr_after);
    } else if corr_after > corr_before {
        println!("PARTIAL: NNUE improved but not great (Spearman = {:.3})", corr_after);
    } else {
