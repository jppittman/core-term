...
use pixelflow_core::{execute, TensorView, TensorViewMut, Batch};
use pixelflow_core::ops::{SampleAtlas, Max};
...
pub fn render_glyph_direct(
    ...
    let glyph_idx = match GLYPH_METADATA.binary_search_by_key(&ch, |meta| meta.c) {
        Ok(idx) => idx,
        Err(_) => return GlyphMetrics { width: 0, height: 0, bearing_x: 0, bearing_y: 0 },
    };
    ...
    let mut screen_view = TensorViewMut::new(target.dest, target.stride, target.dest.len() / target.stride, target.stride);
