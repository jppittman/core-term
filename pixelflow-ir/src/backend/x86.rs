...
impl SimdOps for F32x4 {
    ...
    fn log2(self) -> Self {
        ...
        unsafe {
            ...
            // Extract exponent using integer shifts
            // (x >> 23) & 0xFF - 127
            let exp_bits = _mm_srli_epi32(x_i32, 23);
            let exp_mask = _mm_set1_epi32(0xFF);
            let biased_exp = _mm_and_si128(exp_bits, exp_mask);
            let bias = _mm_set1_epi32(127);
            let unbiased_exp = _mm_sub_epi32(biased_exp, bias);
            let mut n = _mm_cvtepi32_ps(unbiased_exp);

            // Extract mantissa in [1, 2)
            let mant_mask = _mm_set1_epi32(0x007FFFFF_u32 as i32);
            let one_bits = _mm_set1_epi32(0x3F800000_u32 as i32); // 1.0
            ...
            // Max error: ~1e-4
            let c4 = _mm_set1_ps(-0.320044);
            let c3 = _mm_set1_ps(1.797497);
            let c2 = _mm_set1_ps(-4.198805);
            let c1 = _mm_set1_ps(5.727023);
            let c0 = _mm_set1_ps(-3.005615);
    ...
    fn exp2(self) -> Self {
        ...
        unsafe {
            ...
            let c4 = _mm_set1_ps(0.013556);
            let c3 = _mm_set1_ps(0.052032);
            let c2 = _mm_set1_ps(0.241379);
...
impl SimdOps for F32x8 {
    ...
    fn log2(self) -> Self {
        unsafe {
            ...
            // Extract exponent using integer shifts
            // bits 23-30 are exponent. Shift right by 23.
            let exp_bits = _mm256_srli_epi32(x_i32, 23);
            let exp_mask = _mm256_set1_epi32(0xFF);
            let biased_exp = _mm256_and_si256(exp_bits, exp_mask);
            let bias = _mm256_set1_epi32(127);
            let unbiased_exp = _mm256_sub_epi32(biased_exp, bias);

            // Convert integer exponent to float
            let mut n = _mm256_cvtepi32_ps(unbiased_exp);

            // Extract mantissa in [1, 2)
            let mant_mask = _mm256_set1_epi32(0x007FFFFF_u32 as i32);
            let one_bits = _mm256_set1_epi32(0x3F800000_u32 as i32); // 1.0
            let mantissa_bits = _mm256_and_si256(x_i32, mant_mask);
            let mut f = _mm256_castsi256_ps(_mm256_or_si256(mantissa_bits, one_bits));
            ...
            // Fitted using least squares on Chebyshev nodes
            // Max error: ~1e-4
            let c4 = _mm256_set1_ps(-0.320044);
            let c3 = _mm256_set1_ps(1.797497);
            let c2 = _mm256_set1_ps(-4.198805);
            let c1 = _mm256_set1_ps(5.727023);
            let c0 = _mm256_set1_ps(-3.005615);
    ...
    fn exp2(self) -> Self {
        unsafe {
            ...
            // Minimax polynomial for 2^f, f ∈ [0, 1)
            let c4 = _mm256_set1_ps(0.013556);
            let c3 = _mm256_set1_ps(0.052032);
            let c2 = _mm256_set1_ps(0.241379);
...
impl SimdOps for F32x16 {
    ...
    fn mask_to_float(mask: Mask16) -> Self {
        // Convert k-register to float representation (all-1s or all-0s per lane)
    ...
    fn log2(self) -> Self {
        unsafe {
            ...
            // Fitted using least squares on Chebyshev nodes
            // Max error: ~1e-4
            let c4 = _mm512_set1_ps(-0.320044);
            let c3 = _mm512_set1_ps(1.797497);
            let c2 = _mm512_set1_ps(-4.198805);
            let c1 = _mm512_set1_ps(5.727023);
            let c0 = _mm512_set1_ps(-3.005615);
    ...
    fn exp2(self) -> Self {
        ...
        unsafe {
            ...
            // Minimax polynomial for 2^f, f ∈ [0, 1)
            // Degree 4, max error ~10^-7
            let c4 = _mm512_set1_ps(0.013556);
            let c3 = _mm512_set1_ps(0.052032);
            let c2 = _mm512_set1_ps(0.241379);
