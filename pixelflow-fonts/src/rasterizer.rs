...
impl<'a> Surface<u8> for GlyphSurface<'a> {
    fn eval(&self, x: Batch<u32>, y: Batch<u32>) -> Batch<u8> {
        ...
        let bx = self.glyph.bounds.bearing_x as f32;
        let by_top = self.glyph.bounds.bearing_y as f32;
        let by_bottom = by_top - self.glyph.bounds.height as f32;
        let w = self.glyph.bounds.width as f32;
...
impl GlyphBuilder {
    ...
    fn subdivide_cubic(&mut self, p0: Point, p1: Point, p2: Point, p3: Point, depth: u32) {
        ...
        // If segment is very short, emit Line.
        // Also verify flatness (control points near chord).
        // For now simple length check + depth limit.
