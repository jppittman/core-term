...
fn test_sqrt_derivative() {
    ...
    assert!(val_diff.lt(Field::from(1e-3)).all(), "Value mismatch: expected 2.0, got {:?}", result.val());

    // Derivative check: 1/(2*sqrt(4)) = 0.25
    let dx_diff = (result.dx() - Field::from(0.25)).constant().abs();
    assert!(dx_diff.lt(Field::from(1e-3)).all(), "Derivative mismatch: expected 0.25, got {:?}", result.dx());
}
...
fn test_sqrt_derivative_zero() {
    ...
    assert!(val_diff.lt(Field::from(1e-3)).all(), "Value mismatch at 0: expected 0.0, got {:?}", result.val());
}
