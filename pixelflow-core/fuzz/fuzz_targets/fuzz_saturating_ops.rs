...
fuzz_target!(|data: &[u8]| {
    ...
    if a == u32::MAX {
        ...
        let add_max_arr = add_max.to_array_usize();
        assert_eq!(add_max_arr[0], u32::MAX as usize, "MAX + 1 should saturate to MAX");
    }

    if a == 0 {
        let sub_zero = a_batch.saturating_sub(Batch::splat(1));
        let sub_zero_arr = sub_zero.to_array_usize();
