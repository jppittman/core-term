...
/// - Scalar: `MaskScalar`
#[cfg(all(target_arch = "x86_64", target_feature = "avx512f", pixelflow_avx512f))]
/// AVX-512 mask storage (16-bit mask register).
...
/// AVX2 mask storage (8-bit mask encoded in YMM register).
...
/// SSE2 mask storage (4-bit mask encoded in XMM register).
pub type NativeMaskStorage = crate::backend::x86::Mask4;

#[cfg(target_arch = "aarch64")]
/// NEON mask storage (128-bit vector mask).
pub type NativeMaskStorage = crate::backend::arm::Mask4;

#[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
/// Scalar mask storage (bool).
