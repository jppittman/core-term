...
impl Div for SimdVec<u32> {
    ...
    fn div(self, rhs: Self) -> Self {
        let mut a = [0u32; 4];
        let mut b = [0u32; 4];
        // Use inherent store if visible, or generic store logic
        unsafe {
             _mm_storeu_si128(a.as_mut_ptr() as *mut _, self.0);
             _mm_storeu_si128(b.as_mut_ptr() as *mut _, rhs.0);
        }
...
impl BatchArithmetic<u32> for SimdVec<u32> {
    ...
    fn gather(base: &[u32], indices: Self) -> Self {
        unsafe {
            ...
            let v0 = base.get_unchecked(idx[0] as usize);
            let v1 = base.get_unchecked(idx[1] as usize);
            let v2 = base.get_unchecked(idx[2] as usize);
            let v3 = base.get_unchecked(idx[3] as usize);
