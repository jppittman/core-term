...
// NOTE: No Field4 impl needed - the generic impl handles all domains where
// M's output has derivatives. For Field4, there's no such M, so ValOf
// simply doesn't implement Manifold<Field4>. This is correct: V(expr)
// only makes sense when expr returns a Jet type.
