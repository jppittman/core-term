...
impl SimdOps for F32x4 {
    ...
    fn log2(self) -> Self {
        ...
        unsafe {
            ...
            // Correct exponent extraction: shift and subtract bias
            let exp_i32 = _mm_srli_epi32(x_i32, 23);
            let exp_val = _mm_sub_epi32(exp_i32, _mm_set1_epi32(127));
            ...
            let mant_bits = _mm_or_si128(_mm_and_si128(x_i32, mant_mask), one_bits);
            let mut f = _mm_castsi128_ps(mant_bits);
...
impl SimdOps for F32x8 {
    ...
    fn log2(self) -> Self {
        unsafe {
            ...
            // Correct exponent extraction: shift and subtract bias
            let exp_i32 = _mm256_srli_epi32(x_i32, 23);
            let exp_val = _mm256_sub_epi32(exp_i32, _mm256_set1_epi32(127));
            let mut n = _mm256_cvtepi32_ps(exp_val);

            // Extract mantissa in [1, 2)
            let mant_mask = _mm256_set1_epi32(0x007FFFFF);
            let one_bits = _mm256_set1_epi32(0x3F800000);
            let mant_bits = _mm256_or_si256(_mm256_and_si256(x_i32, mant_mask), one_bits);
            let mut f = _mm256_castsi256_ps(mant_bits);
