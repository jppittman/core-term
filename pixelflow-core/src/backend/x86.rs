...
impl SimdOps for F32x16 {
    ...
    fn simd_log2(self) -> Self {
        ...
        unsafe {
            // Extract mantissa normalized to [1, 2) to match polynomial coefficients
            // Const generics: interval=1 (_MM_MANT_NORM_1_2), sign=0 (_MM_MANT_SIGN_src)
            // - Interval [1, 2) matches SSE2/AVX2/NEON normalization
            // - Sign control: preserve source sign
            let f = _mm512_getmant_ps::<1, 0>(self.0);

            // Extract exponent (automatically adjusts for the mantissa range)
