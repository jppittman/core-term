...
impl SimdOps for F32x4 {
    ...
    fn log2(self) -> Self {
        ...
        unsafe {
            ...
            // Extract exponent directly as integer
            // Bias is 127. If we extract exponent bits and shift down, we get (n + 127).
            // To get n as float, we subtract 127 using integer sub then convert.
            // But doing it all in float domain (like before) failed for 1.0 (got -127).
            // Let's use standard integer extraction which is robust.
            let exp_mask = _mm_set1_epi32(0x7F800000_u32 as i32);
            let raw_exp = _mm_and_si128(x_i32, exp_mask);
            let exp_shifted = _mm_srli_epi32(raw_exp, 23);
            let exp_unbiased = _mm_sub_epi32(exp_shifted, _mm_set1_epi32(127));
            let mut n = _mm_cvtepi32_ps(exp_unbiased);

            // Extract mantissa in [1, 2) by forcing exponent to 127 (1.0)
...
impl SimdOps for F32x8 {
    ...
    fn log2(self) -> Self {
        unsafe {
            ...
            let exp_shifted = _mm256_srli_epi32(raw_exp, 23);
            let exp_unbiased = _mm256_sub_epi32(exp_shifted, _mm256_set1_epi32(127));
