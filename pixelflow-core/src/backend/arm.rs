...
impl SimdOps for F32x4 {
    ...
    fn simd_log2(self) -> Self {
        ...
        unsafe {
            ...
            let t = vsubq_f32(f, one);

            // Remez minimax polynomial for log2(1+t), t ∈ [0, 1)
            // Approximates: log2(1+t) = t * (c1 + t * (c2 + t * (c3 + t * c4)))
            // Degree 4, max error ~2×10^-4
            let c4 = vdupq_n_f32(-0.3606);
            let c3 = vdupq_n_f32(0.4808);
            let c2 = vdupq_n_f32(-0.7213);
            let c1 = vdupq_n_f32(1.4427); // 1/ln(2)

            // Horner's method using NEON FMA: vfmaq_f32(c, a, b) = a*b + c
            // Compute: t * (c1 + t * (c2 + t * (c3 + t * c4)))
            let poly = vfmaq_f32(c3, c4, t);
            let poly = vfmaq_f32(c2, poly, t);
            let poly = vfmaq_f32(c1, poly, t);
            let poly = vmulq_f32(poly, t); // Final multiply by t
