...
use super::pty::{NixPty, PtyConfig, PtyChannel};
use std::io::{Read, Write, ErrorKind};
use std::thread;
use std::time::Duration;
use nix::sys::signal::{kill, Signal};
use nix::unistd::Pid; // Reinstated for explicit type annotation
...
fn read_from_pty_with_timeout(pty: &mut NixPty, expected_str: &str) -> Result<String, String> {
    ...
    loop {
        if start_time.elapsed() > READ_TIMEOUT {
            return Err(format!("Timeout reading from PTY. Expected to contain '{}', got '{}'", 
                               expected_str, String::from_utf8_lossy(&full_output_bytes)));
        }

        // Check if expected_bytes is already in full_output_bytes
        if full_output_bytes.windows(expected_bytes.len()).any(|window| window == expected_bytes) {
            log::debug!("Expected string '{}' found in accumulated output.", expected_str.trim_end_matches('\n'));
            break;
        }
        
        match pty.read(&mut buffer) {
            Ok(0) => { // EOF
                log::debug!("Read EOF from PTY. Full output: '{}'", String::from_utf8_lossy(&full_output_bytes));
                // Check one last time after EOF
                if full_output_bytes.windows(expected_bytes.len()).any(|window| window == expected_bytes) {
                    break;
                }
                // If EOF and string not found, it's an error.
                return Err(format!("EOF reached but expected string '{}' not found in output '{}'", 
                                   expected_str, String::from_utf8_lossy(&full_output_bytes)));
            }
            Ok(bytes_read) => {
                log::debug!("Read {} bytes: '{}'", bytes_read, String::from_utf8_lossy(&buffer[..bytes_read]).trim_end_matches('\n'));
                full_output_bytes.extend_from_slice(&buffer[..bytes_read]);
                // Check again after new data
                if full_output_bytes.windows(expected_bytes.len()).any(|window| window == expected_bytes) {
                    log::debug!("Expected string '{}' found after appending new data. Breaking.", expected_str.trim_end_matches('\n'));
                    break; 
                }
            }
            Err(e) if e.kind() == ErrorKind::WouldBlock => {
                log::trace!("Read WouldBlock, retrying. Accumulated: '{}'", String::from_utf8_lossy(&full_output_bytes));
                thread::sleep(Duration::from_millis(100)); // Wait a bit before retrying
                continue;
            }
            Err(e) => { // Other errors (like EIO)
                ...
                return Err(format!("Error reading from PTY: {:?}. Partial output: '{}'", e, String::from_utf8_lossy(&full_output_bytes)));
            }
        }
    }
...
fn test_pty_spawn_successful() {
    ...
    match NixPty::spawn_with_config(&config) {
        Ok(mut pty) => {
            log::debug!("Successfully spawned PTY for echo test, child PID: {}", pty.child_pid());
            ...
            let output = read_from_pty_with_timeout(&mut pty, expected_output)
                .unwrap_or_else(|e| panic!("pty_spawn_successful: Failed to read from PTY. Error: {}", e));
...
fn test_pty_read_write_interaction() {
    ...
    let mut pty = match NixPty::spawn_with_config(&config) {
        Ok(p) => p,
        Err(e) => panic!("test_pty_read_write_interaction: Failed to spawn PTY: {:?}", e),
    };
    log::debug!("Spawned PTY for read/write test, child PID: {}", pty.child_pid());
    ...
    log::info!("Read/write test successful. Full output: '{}'", output.trim());
    // NixPty instance is dropped here.
}
...
fn test_pty_resize_successful() {
    ...
    log::debug!("Spawned PTY for resize test, child PID: {}", pty.child_pid());

    let new_cols = 100;
    let new_rows = 30;
    match pty.resize(new_cols, new_rows) {
        Ok(()) => {
            log::info!("Successfully resized PTY for child PID {} to {}x{}", pty.child_pid(), new_cols, new_rows);
...
fn test_pty_child_termination_on_drop() {
    ...
    let pty = match NixPty::spawn_with_config(&config) {
        Ok(p) => p,
        Err(e) => panic!("test_pty_child_termination_on_drop: Failed to spawn PTY: {:?}", e),
    };
    
    let child_pid: Pid = pty.child_pid(); // Capture PID before pty is dropped
    log::debug!("Spawned PTY for child termination test, child PID: {}", child_pid);
    ...
    match kill(child_pid, None) { 
        Ok(_) => {
            ...
            log::warn!("Child process {} still alive after drop and SIGHUP. Sending SIGKILL.", child_pid);
        ...
        Err(nix::Error::ESRCH) => {
            ...
            log::info!("Child process {} successfully terminated after PTY drop.", child_pid);
        }
        Err(e) => {
            // Other errors from kill check.
            panic!("test_pty_child_termination_on_drop: Error checking child process {}: {:?}", child_pid, e);
        }
    }
}
        ...
        Ok(mut pty) => {
            ...
            loop {
                ...
                match pty.read(&mut read_buffer) {
                    ...
                    Err(e) if e.kind() == ErrorKind::WouldBlock => {
                        log::trace!("test_pty_spawn_invalid_command: Read WouldBlock, attempt {}.", read_attempts);
