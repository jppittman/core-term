...
mod orchestrator_tests {
    ...
    use crate::term::{
        ControlEvent, EmulatorAction, EmulatorInput, TerminalEmulator, TerminalInterface,
    };
    ...
    impl MockPtyChannel {
        fn new() -> Self {
            Self {
                ...
                child_pid_val: Pid::from_raw(1234), // Dummy PID
    ...
    impl AnsiParserTrait for MockAnsiParser {
        fn process_bytes(&mut self, bytes: &[u8]) -> Vec<AnsiCommand> {
            self.bytes_processed_log
                ...
                .unwrap()
                .push(bytes.to_vec());
            // Return all expected commands for this specific call
            self.commands_to_return_on_next_call.drain(..).collect()
        }
    }

    // --- Mock TerminalInterface (TerminalEmulator) ---
    #[derive(Clone)]
    struct MockTerminal {
        ...
        scrollback_limit: usize, // Usually not directly relevant for orchestrator tests
    ...
    impl MockTerminal {
        ...
        fn expect_action_for_next_call(&self, action: Option<EmulatorAction>) {
        ...
        fn clear_all_dirty_lines(&mut self) {
            self.dirty_lines.lock().unwrap().clear();
        }
    }
    ...
    impl TerminalInterface for MockTerminal {
        ...
        fn interpret_input(&mut self, input: EmulatorInput) -> Option<EmulatorAction> {
            ...
            match input {
                ...
                // Add more specific state changes if a test requires the mock terminal
                // to react in a certain way to a specific input to then produce a specific action.
                _ => {}
            }
            self.actions_to_return_on_next_call
                .lock()
    ...
    impl MockDriver {
        ...
        fn expect_events_for_next_call(&self, events: Vec<BackendEvent>) {
    ...
    fn test_orchestrator_pty_data_flow_exact_verification() {
        ...
        // Configure parser to return specific commands for this input
        ...
        // Configure terminal to return a specific action when it processes these commands
        // For simplicity, let's say processing 'H' returns an action, 'i' returns None.
        ...
        mock_term.expect_action_for_next_call(None); // For 'i'
        ...
        // Verify parser interaction
        let parsed_bytes_log = mock_parser.get_processed_bytes_log();
        assert_eq!(
            parsed_bytes_log,
        ...
        let inputs_to_term = mock_term.get_inputs_received();
        let expected_inputs_to_term: Vec<EmulatorInput> = expected_parsed_commands
            .into_iter()
            .map(EmulatorInput::Ansi)
            .collect();
        assert_eq!(
            inputs_to_term, expected_inputs_to_term,
            "Terminal did not receive exact parsed commands"
        );

        // Verify PTY write interaction (due to action from terminal)
        let pty_writes = mock_pty.get_written_data();
        assert_eq!(
            pty_writes, b"ack_H",
    ...
    fn test_orchestrator_driver_key_event_flow_exact_verification() {
        ...
        // Verify PTY write interaction
        let pty_writes = mock_pty.get_written_data();
        assert_eq!(
            pty_writes, b"X_response",
    ...
    fn test_orchestrator_driver_resize_event_flow_exact_verification() {
        ...
        let renderer = Renderer::new();
        let mut mock_driver = MockDriver::new();
        mock_driver.font_dims = (10, 20); // Font: 10px wide, 20px high
        ...
        // Terminal's interpret_input for Resize usually returns None, state change is internal.
        ...
        // Verify PTY resize call
        let pty_resize_calls = mock_pty.get_resize_calls();
        assert_eq!(
            pty_resize_calls,
        ...
        // Verify terminal dimensions were updated by the mock's interpret_input
        assert_eq!(
    ...
    fn test_render_if_needed_no_dirty_lines_no_first_draw() {
        ...
        renderer.first_draw = false; // Not the first draw
        let mut mock_driver = MockDriver::new();

        // Ensure no lines are dirty in the mock terminal
        ...
        let driver_calls = mock_driver.get_calls();
        // Expect NO drawing calls, not even Present, if nothing was drawn
        assert!(
    ...
    fn test_render_if_needed_first_draw_scenario() {
        let mut mock_pty = MockPtyChannel::new();
        let mut mock_term = MockTerminal::new(2, 1, 0); // Term has 2x1 cells
        let mut mock_parser = MockAnsiParser::new();
        let mut renderer = Renderer::new(); // first_draw is true by default
        let mut mock_driver = MockDriver::new();

        // MockTerminal is initially all dirty, and cursor is at (0,0) visible.
        // Renderer on first_draw will:
        // 1. ClearAll with RENDERER_DEFAULT_BG
        // 2. FillRect for the two space cells with RENDERER_DEFAULT_BG
        // 3. DrawTextRun for cursor at (0,0) (inverted space)
        ...
        let expected_calls = vec![
            ...
            MockDriverCall::FillRect {
                // For the two space cells (0,0) and (1,0)
                rect: CellRect {
                    x: 0,
                    y: 0,
                    width: 2,
            ...
            MockDriverCall::DrawTextRun {
                // Cursor at (0,0) over a space
                coords: CellCoords { x: 0, y: 0 },
                text: " ".to_string(),
                style: TextRunStyle {
                    fg: RENDERER_DEFAULT_BG, // Inverted default FG
                    bg: RENDERER_DEFAULT_FG, // Inverted default BG
                    flags: AttrFlags::empty(),
                },
            },
        ...
        assert!(
            !orchestrator.renderer.first_draw,
    ...
    fn test_orchestrator_handles_emulator_action_set_title_exact() {
        ...
        mock_parser.expect_commands_for_next_call(vec![AnsiCommand::Print('T')]); // Dummy trigger
        mock_term
    ...
    fn test_orchestrator_handles_focus_events_exact() {
        ...
        mock_term.expect_action_for_next_call(None);
        let status_gain = orchestrator.process_driver_events().unwrap();
        ...
        mock_term.expect_action_for_next_call(None);
        let status_lost = orchestrator.process_driver_events().unwrap();
    ...
    fn test_orchestrator_interleaved_pty_and_driver_events() {
        ...
        mock_term.expect_action_for_next_call(Some(EmulatorAction::WritePty(b"AckP1".to_vec()))); // For 'P'
        mock_term.expect_action_for_next_call(None); // For '1'
        assert_eq!(
            orchestrator.process_pty_events().unwrap(),
            OrchestratorStatus::Running
        );
        ...
        mock_parser.bytes_processed_log.lock().unwrap().clear();
        mock_term.clear_inputs_received();
        ...
        mock_term.expect_action_for_next_call(None); // For 'P'
        mock_term
            .expect_action_for_next_call(Some(EmulatorAction::SetTitle("TitleP2".to_string()))); // For '2'
        assert_eq!(
            orchestrator.process_pty_events().unwrap(),
            OrchestratorStatus::Running
        );
        ...
        assert!(mock_pty.get_written_data().is_empty()); // No PTY write this time
        assert_eq!(
            mock_driver.get_calls(),
            vec![MockDriverCall::SetTitle {
