...
impl CocoaDriver {
    ...
    pub fn close_window(&mut self) {
        info!("CocoaDriver: Closing window");
        unsafe {
            // Close the window
            let _: () = msg_send![self.window, close];

            // Stop the application run loop
            let app = NSApp();
            let _: () = msg_send![app, stop: nil];

            // Post a dummy event to wake up the event loop so it can process the stop message
            // Without this, the app might not exit immediately
            let event: id = msg_send![
                class!(NSEvent),
                otherEventWithType: 0u64  // NSEventTypeApplicationDefined
