...
impl X11FontManager {
    ...
    fn find_glyph_in_font(&self, font: &SafeXftFont, char_code: char) -> Option<u32> {
        if font.raw().is_null() {
            warn!(
                "find_glyph_in_font called with a null font pointer for char '{}'.",
                char_code
            );
            return None;
        }
...
impl FontManager for X11FontManager {
    ...
    fn get_font_handle(&self, font_id: usize) -> Self::FontHandle {
        match font_id {
            ...
            id if id >= FALLBACK_FONT_ID_START => {
                ...
                self.fallback_fonts.get(fallback_idx).map_or_else(
                    || {
                        error!(
    ...
    fn get_glyph(&mut self, character: char, attributes: AttrFlags) -> Option<ResolvedGlyph> {
        ...
        let (primary_font, primary_font_id) =
            if attributes.contains(AttrFlags::BOLD | AttrFlags::ITALIC) {
        ...
        if let Some(glyph_id) = self.find_glyph_in_font(primary_font, character) {
            trace!(
                "Found glyph for '{}' in primary font_id {}",
                character, primary_font_id
            );
        ...
        for (idx, fallback_font) in self.fallback_fonts.iter().enumerate() {
            if let Some(glyph_id) = self.find_glyph_in_font(fallback_font, character) {
                ...
                trace!(
                    "Found glyph for '{}' in cached fallback font_id {}",
                    character, font_id
                );
        ...
        trace!(
            "Glyph for '{}' not in primary or cached fallbacks. Querying Fontconfig.",
            character
        );
        ...
        unsafe {
            ...
            if matched_pattern_xft.is_null() {
                trace!(
                    "XftFontMatch found no fallback font for char '{}'",
                    character
                );
                return None;
            }
