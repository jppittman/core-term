...
use crate::platform::backends::{BackendEvent, Driver, PlatformState, CursorVisibility};
...
impl LinuxWaylandPlatform {
    pub fn new(
        ...
        let pty = NixPty::spawn_with_config(&pty_config).context("Failed to create NixPty for Wayland")?;
        let mut driver = WaylandDriver::new().context("Failed to create WaylandDriver")?;
        let event_monitor = EventMonitor::new().context("Failed to create EventMonitor for Wayland")?;
        ...
        if let Some(driver_fd) = driver.get_event_fd() {
            ...
            warn!("Wayland Driver does not provide an event FD for polling. UI event processing might be impacted.");
        }

        let initial_platform_state = driver.get_platform_state();
        info!("Wayland: Initial platform state: {:?}", initial_platform_state);
                    ...
                    match self.driver.process_events() {
                        Ok(driver_backend_events) => {
                            for backend_event in driver_backend_events {
                                self.platform_events.push(PlatformEvent::from(backend_event));
                            }
                        }
                        Err(e) => {
                            error!("Wayland driver event processing failed: {:?}. Requesting close.", e);
                            self.platform_events.push(PlatformEvent::from(BackendEvent::CloseRequested));
                ...
                PTY_EPOLL_TOKEN => {
                    ...
                    loop {
                        match self.pty.read(&mut pty_read_chunk_buf) {
                            ...
                            Ok(count) => {
                                if count > 0 {
                                    accumulated_pty_data.extend_from_slice(&pty_read_chunk_buf[..count]);
                                    pty_bytes_read_this_batch += count;
                                } else { break; } // No more data in this chunk read
                            }
                            Err(e) if e.kind() == ErrorKind::WouldBlock => break, // Done reading for now
                            Err(e) if e.kind() == ErrorKind::Interrupted => continue, // Retry read
                            Err(e) => {
                                error!("Wayland: Critical PTY read error: {:?}. Requesting close.", e);
                                self.platform_events.push(PlatformEvent::from(BackendEvent::CloseRequested));
    ...
    pub fn shutdown(&mut self) -> Result<()> {
        ...
        self.driver.cleanup().context("Failed to cleanup WaylandDriver during shutdown")?;
...
impl Platform for LinuxWaylandPlatform {
    fn new(
        ...
        shell_command: String,
        shell_args: Vec<String>,
    ) -> Result<(Self, PlatformState)> where Self: Sized {
        LinuxWaylandPlatform::new(initial_pty_cols, initial_pty_rows, shell_command, shell_args)
    }
    ...
    fn poll_events(&mut self) -> Result<Vec<PlatformEvent>> {
        ...
        loop {
            ...
            if elapsed_in_burst >= max_burst_duration { break; }

            let remaining_time_in_burst = max_burst_duration.saturating_sub(elapsed_in_burst);
            let epoll_timeout_ms = std::cmp::min(
                min_latency_ms,
                remaining_time_in_burst.as_millis().try_into().unwrap_or(min_latency_ms)
            ).max(0); // Ensure timeout is not negative

            self.epoll_event_buffer.clear(); // Clear before waiting for new events
            match self.event_monitor.events(&mut self.epoll_event_buffer, epoll_timeout_ms) {
                Ok(()) => { // Ok(()) means epoll_wait returned successfully, number of events is in buffer len
                    if self.epoll_event_buffer.is_empty() { // Timeout occurred, no events
                        break;
                    }
                ...
                Err(e) => {
                    if let Some(nix_err) = e.downcast_ref::<nix::Error>() {
                        if *nix_err == nix::Error::EINTR {
                            trace!("Wayland: epoll_wait interrupted (EINTR), continuing poll loop.");
        ...
        if !accumulated_pty_data.is_empty() {
            self.platform_events.push(PlatformEvent::IOEvent { data: accumulated_pty_data });
        }
        ...
        for action in actions {
            match action {
                PlatformAction::Write(data) => {
                    self.pty.write_all(&data).context("Wayland: Failed to write to PTY")?;
                }
                PlatformAction::ResizePty { cols, rows } => {
                    self.pty.resize(cols, rows).context("Wayland: Failed to resize PTY")?;
                }
                PlatformAction::Render(commands) => {
                    ...
                    self.driver.present().context("Wayland: Failed to present frame")?;
                }
                ...
                PlatformAction::CopyToClipboard(text) => {
                    debug!("Wayland: PlatformAction::CopyToClipboard: '{}'. (Not fully implemented)", text);
    ...
    fn test_linux_wayland_platform_new() -> Result<()> {
        ...
        match LinuxWaylandPlatform::new(80, 24, shell_command, shell_args) {
            Ok((mut platform, initial_state)) => {
                ...
                assert_eq!(current_state, initial_state, "Initial and current states should match before any operations.");
