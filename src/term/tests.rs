...
use crate::glyph::{Cell, CellAttrs, Flags, Fill}; // Assuming Fill is part of public API for cell comparison
    ...
    for r in 0..snapshot.num_rows {
        ...
        for (char_idx, expected_char) in expected_row_str.chars().enumerate() {
            if s_col >= snapshot.num_cols {
                panic!("Expected row string '{}' is wider than snapshot num_cols {}", expected_row_str, snapshot.num_cols);
            }
            ...
            assert_eq!(
                ...
                "Char mismatch at (row {}, col_char_idx {})", r, char_idx 
            );
        ...
        for c_fill in s_col..snapshot.num_cols {
            ...
            assert_eq!(cell.char, ' ', "Expected empty char for fill at ({}, {})", r, c_fill);
        }
    }
...
fn test_csi_ed_clear_below_csi_j() {
    ...
    assert_eq!(get_cell_from_snapshot(&snapshot_before, 0,0).unwrap().char, 'A');
    assert_eq!(get_cell_from_snapshot(&snapshot_before, 1,0).unwrap().char, 'D');
    ...
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 0, 0).unwrap().char, 'A');
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 0, 1).unwrap().char, 'B');
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 0, 2).unwrap().char, 'C');

    // Line 1 from cursor onwards should be cleared
    // Cell (1,0) 'D' should be cleared because cursor is on it.
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 1, 0).unwrap().char, ' ');
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 1, 1).unwrap().char, ' ');
    assert_eq!(get_cell_from_snapshot(&snapshot_after, 1, 2).unwrap().char, ' ');
...
fn test_csi_sgr_fg_color() {
    ...
    assert_ne!(cell_a.attrs.fg, CellAttrs::default().fg, "Foreground color should have changed");
    ...
    assert_eq!(cell_b.attrs.fg, CellAttrs::default().fg, "Foreground color should have reset");
}
...
fn test_key_event_arrow_up() {
    ...
    assert!(written_to_pty == Some("\x1b[A".to_string()) || written_to_pty == Some("\x1bOA".to_string()));
...
fn test_set_selection_and_clear() {
    ...
    let selection = SelectionRange {
        start_row: 0, start_col: 1, // From '1'
        end_row: 1, end_col: 3,     // To 'C' (inclusive end for range definition)
        is_block: false,
    };
...
fn test_mode_show_cursor_deccm() {
    ...
    assert_ne!(snap_default.cursor.style.shape, CursorShape::Hidden, "Cursor should be visible by default");
    ...
    assert_eq!(snap_hidden.cursor.style.shape, CursorShape::Hidden, "Cursor should be hidden after DECRST ?25l");

    // Show cursor: DECSET ?25h
    term.process_input(b"\x1b[?25h");
    let snap_shown = term.get_render_snapshot();
    assert_ne!(snap_shown.cursor.style.shape, CursorShape::Hidden, "Cursor should be visible again after DECSET ?25h");
    // It should revert to its previous non-hidden shape, e.g. Block
    assert_eq!(snap_shown.cursor.style.shape, CursorShape::Block, "Cursor should revert to Block shape");
}
