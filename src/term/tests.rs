...
fn test_ps1_multiline_at_bottom_of_scrolling_region() {
    ...
    // Cursor moves to (0,0) logical within region, which is (phys 1,0)
    let snap_after_stbm = term.get_render_snapshot();
    assert_screen_state(&snap_after_stbm, &["     ","     ","     ","     "], Some((1,0)));


    // Line 0: "XXXXX" (outside scroll region, printed before STBM)
    // To simplify, let's print after STBM and move cursor back up.
    ...
    // Screen: XXXXX\n     \n     \n     , cursor (phys 0,5) -> (phys 0,4) after wrap
    ...
    // Cursor moves to start of phys row 3 (which is logical row 2 relative to region top, if origin mode was on)
    // Since origin mode is off by default, LF + CR means cursor moves to (phys 3,0)
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::C0Control(C0Control::LF)));
...
fn test_ps1_multiline_prompt_overflows_tiny_screen_repeatedly() {
    ...
    let snapshot = term.get_render_snapshot();
    assert_screen_state(&snapshot, &["C ", "> "], Some((1,1))); // Cursor is at (1,1) after printing space makes it (1,2) logical, but physical is (1,1) because space is 1 wide.
                                                                    // The `cursor_wrap_next` should be true.
    assert!(term.cursor_wrap_next, "cursor_wrap_next should be true after prompt fills line");
