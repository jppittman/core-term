...
/// Respects origin mode via effective_top/effective_bottom.
...
pub(super) fn handle_printable(term: &mut Term, c: char) {
    trace!("handle_printable: char='{}', cursor=({}, {})", c, term.cursor.x, term.cursor.y);
    // TODO(#1): Implement proper character width calculation.
    let char_width = 1;

    // TODO(#2): Add check for DECAWM mode.
    let needs_wrap = term.cursor.x + char_width > term.width;
    if needs_wrap {
        term.cursor.x = 0;
        index(term);
    }
    ...
    if let Some(row) = current_screen.get_mut(term.cursor.y) {
        ...
        if char_width > 1 {
             if let Some(_next_cell) = row.get_mut(term.cursor.x + 1) {
                 // TODO(#1): Implement wide char dummy logic
             }
        }
    }
...
/// Moves cursor to column 0 (CR - Carriage Return).
pub(super) fn carriage_return(term: &mut Term) {
     trace!("carriage_return: cursor=({}, {})", term.cursor.x, term.cursor.y);
    term.cursor.x = 0;
}

/// Moves cursor left one column, stopping at column 0 (BS - Backspace).
...
/// Moves cursor to the next tab stop (HT - Horizontal Tabulation).
