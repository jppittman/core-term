...
//!
//! Uses `wayland-client` and `calloop` to implement a native Wayland backend.
//! Handles SHM buffer management, input events (via `xkbcommon`), and window lifecycle.
...
impl DisplayDriver for WaylandDisplayDriver {
    ...
    fn new(engine_tx: EngineSender<Bgra>) -> Result<Self> {
        let (cmd_tx, channel) = calloop::channel::channel();

        Ok(Self {
            cmd_tx,
            run_state: Arc::new(Mutex::new(Some(RunState { channel, engine_tx }))),
        })
    }
    ...
    fn send(&self, cmd: DriverCommand<Bgra>) -> Result<()> {
        self.cmd_tx
            .send(cmd)
            .map_err(|_| anyhow!("Wayland driver channel disconnected"))
    }

    fn run(&self) -> Result<()> {
        // Steal the run_state (only one thread can run)
        let run_state = self
            .run_state
            .lock()
            .unwrap()
            .take()
            .ok_or_else(|| anyhow!("Wayland driver already running or this is a clone"))?;
        ...
        // Initialize State
        let mut state = WaylandState::new(conn.clone(), qh.clone(), run_state.engine_tx)?;
        ...
        ).map_err(|e| anyhow!("Failed to insert wayland source: {}", e.error))?;

        // Register Command Channel
        loop_handle.insert_source(run_state.channel, |event, _meta, state| {
            if let ChannelEvent::Msg(cmd) = event {
                state.handle_command(cmd);
            }
        }).map_err(|e| anyhow!("Failed to insert command channel: {}", e.error))?;
...
impl WaylandSource {
    fn new(conn: Connection, queue: wayland_client::EventQueue<WaylandState>) -> Self {
...
impl calloop::EventSource for WaylandSource {
    type Event = ();
    type Metadata = wayland_client::EventQueue<WaylandState>;
    type Ret = ();
    ...
    fn register(&mut self, poll: &mut calloop::Poll, token_factory: &mut calloop::TokenFactory) -> calloop::Result<()> {
        ...
        #[allow(unused_unsafe)]
        unsafe {
            poll.register(borrowed_fd, calloop::Interest::READ, calloop::Mode::Level, token_factory.token())
        }
    }
    ...
    fn reregister(&mut self, poll: &mut calloop::Poll, token_factory: &mut calloop::TokenFactory) -> calloop::Result<()> {
        ...
        #[allow(unused_unsafe)]
        unsafe {
            poll.reregister(borrowed_fd, calloop::Interest::READ, calloop::Mode::Level, token_factory.token())
        }
    }
...
struct Slot {
    ...
    pool: wl_shm_pool::WlShmPool, // Keep pool alive
    canvas: memmap2::MmapMut,
    width: u32,
    height: u32,
    free: bool,
}
...
impl WaylandState {
    fn new(
        ...
        Ok(Self {
            ...
            xdg_surface: None,
            toplevel: None,
            window_id: WindowId(0), // Will be set on creation
            configured: false,
            width: 800,
            height: 600,
            pointer: None,
    ...
    fn present_frame(&mut self, frame: Frame<Bgra>) {
        ...
        unsafe {
            ...
            let dst_ptr = slot.canvas.as_mut_ptr();
    ...
    fn create_shm_buffer(&self, width: u32, height: u32) -> Result<Slot> {
        ...
        let file = tempfile().context("Failed to create temp file")?;
        file.set_len(size as u64).context("Failed to resize file")?;
        ...
        let canvas = unsafe {
            memmap2::MmapMut::map_mut(&file).context("Failed to mmap")?
        };
    ...
    fn event(
        ...
        match event {
            wl_pointer::Event::Enter { serial, surface_x: _, surface_y: _, surface: _ } => {
                // Set cursor
                if let (Some(theme), Some(cursor_surface)) = (&mut state.cursor_theme, &state.cursor_surface) {
                    if let Some(cursor) = theme.get_cursor("left_ptr") {
                        let image = &cursor[0];
                        let (width, height) = image.dimensions();
                        let (hx, hy) = image.hotspot();

                        // Destroy old cursor buffer if it exists
                        if let Some(old) = state.cursor_slot.take() {
                            old.buffer.destroy();
                            old.pool.destroy();
                        }

                        // Create new buffer manually
                        if let Ok(mut slot) = state.create_shm_buffer(width, height) {
                             let data: &[u8] = image.as_ref();
                             if slot.canvas.len() >= data.len() {
                                 unsafe {
                                     std::ptr::copy_nonoverlapping(data.as_ptr(), slot.canvas.as_mut_ptr(), data.len());
                                 }
                                 cursor_surface.attach(Some(&slot.buffer), 0, 0);
        ...
        match event {
            wl_keyboard::Event::Keymap { format, fd, size } => {
                if format == WEnum::Value(wl_keyboard::KeymapFormat::XkbV1) {
                    unsafe {
                        let map = memmap2::MmapOptions::new().len(size as usize).map(&fd);
                        if let Ok(map) = map {
