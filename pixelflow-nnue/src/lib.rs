...
fn extract_features_recursive(
    ...
    match expr {
        ...
        Expr::Binary(_, a, b) => {
            ...
            extract_features_recursive(b, features, (_path << 1) | 1, depth.saturating_add(1));
        }
...
impl Nnue {
    ...
    pub fn forward_dense(&self, features: &DenseFeatures) -> Vec<i32> {
        ...
        for (i, &feat_val) in features.values.iter().enumerate() {
            ...
            for (j, out) in output.iter_mut().enumerate().take(dense_size) {
                *out += (self.w_dense[offset + j] as i32) * feat_val;
            }
        }
...
impl Accumulator {
    ...
    pub fn forward_hybrid(&self, nnue: &Nnue, dense_features: &DenseFeatures) -> i32 {
        ...
        for i in 0..l1_size {
            ...
            for (j, val) in l2.iter_mut().enumerate().take(l2_size) {
                *val += (a as i32) * (nnue.w2[i * l2_size + j] as i32);
            }
        }

        // Dense branch contribution (offset by l1_size in W2)
        for (i, &dense_val) in dense_out.iter().enumerate().take(dense_size) {
            let a = (dense_val >> 6).clamp(0, 127) as i8;
            let w2_offset = (l1_size + i) * l2_size;
            for (j, val) in l2.iter_mut().enumerate().take(l2_size) {
                *val += (a as i32) * (nnue.w2[w2_offset + j] as i32);
            }
        }
