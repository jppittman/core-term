...
// ============================================================================
// POTENTIAL BUG: Rapid Start/Stop Around Ticks
// Can rapid start/stop cause missed or duplicate ticks?
// ============================================================================

#[test]
fn rapid_start_stop_does_not_corrupt_tick_count() {
    let tick_count = Arc::new(AtomicUsize::new(0));
    let tick_count_clone = tick_count.clone();
    ...
    let handle = thread::spawn(move || {
        ...
        impl Actor<RenderedResponse, VsyncCommand, VsyncManagement> for RapidActor {
            fn handle_data(&mut self, _: RenderedResponse) {}
            fn handle_control(&mut self, cmd: VsyncCommand) {
                match cmd {
                    VsyncCommand::Start => self.running = true,
        ...
        rx.run(&mut RapidActor {
            tick_count: tick_count_clone,
            running: false,
        });
    });

    // Rapid start/stop with ticks interspersed
    for _ in 0..100 {
        tx.send(Message::Control(VsyncCommand::Start)).unwrap();
        tx.send(Message::Management(VsyncManagement::Tick)).unwrap();
