...
fn clock_thread_stops_when_channel_closed() {
    ...
    let mut builder = ActorBuilder::<RenderedResponse, VsyncCommand, VsyncManagement>::new(10, None);
    let tx = builder.add_producer();
    let clock_tx = builder.add_producer();
    let rx = builder.build_with_burst(100, actor_scheduler::ShutdownMode::default());
