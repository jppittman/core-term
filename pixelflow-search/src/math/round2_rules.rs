...
impl Rewrite for MinMaxDuality {
    ...
    fn apply(&self, _egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        let dual_op = ops::op_from_kind(self.dual)
            .unwrap_or_else(|| panic!("MinMaxDuality: {:?} has no Op", self.dual));
        Some(RewriteAction::NegBinaryNeg {
            op: dual_op,
            a: children[0],
            b: children[1],
        })
...
impl Rewrite for SelectHoistUnary {
    ...
    fn apply(&self, egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        let func_op = ops::op_from_kind(self.func)
            .unwrap_or_else(|| panic!("SelectHoistUnary: {:?} has no Op", self.func));
        Some(RewriteAction::HoistUnaryThroughSelect {
            func: func_op,
            mask: m,
            a,
            b,
...
impl Rewrite for TanDefinition {
    ...
    fn apply(&self, _egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::TanFromSinCos { x: children[0] })
    }
...
impl Rewrite for ExpAsExp2 {
    ...
    fn apply(&self, _egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::ScaleThenUnary {
            scale: LOG2_E,
            func: &ops::Exp2,
            x: children[0],
        })
...
impl Rewrite for LnAsLog2 {
    ...
    fn apply(&self, _egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::UnaryThenScale {
            func: &ops::Log2,
            scale: LN_2,
            x: children[0],
        })
...
impl Rewrite for SqrtProduct {
    ...
    fn apply(&self, egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::CombineThenUnary {
            combine: &ops::Mul,
            func: &ops::Sqrt,
            x,
            y,
...
impl Rewrite for RecipOfProduct {
    ...
    fn apply(&self, egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::UnaryPairThenCombine {
            func: &ops::Recip,
            combine: &ops::Mul,
            x: a,
            y: b,
...
impl Rewrite for FmaUnfuse {
    ...
    fn apply(&self, _egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::FuseThenCombine {
            inner: &ops::Mul,
            outer: &ops::Add,
            a: children[0],
            b: children[1],
...
impl Rewrite for NegDistributesMul {
    ...
    fn apply(&self, egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        let (a, b) = extract_binary(egraph, children[0], OpKind::Mul)?;
        Some(RewriteAction::UnaryThenBinaryWithOther {
            func: &ops::Neg,
            binary: &ops::Mul,
            base: a,
            other: b,
...
impl Rewrite for DivByLiteral {
    ...
    fn apply(&self, egraph: &EGraph, _id: EClassId, node: &ENode) -> Option<RewriteAction> {
        ...
        Some(RewriteAction::MulByLiteralRecip {
            a: children[0],
            recip_k: 1.0 / k,
        })
    }
