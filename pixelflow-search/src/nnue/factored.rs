...
mod tests {
    ...
    /// `bootstrap_extraction_head`'s cold start now seeds from
    /// [`OpEmbeddings::new_with_latency_prior`] instead of fully-random
    /// [`OpEmbeddings::new_random`] — the whole point being to hand the
    /// trainer a monotone latency signal in dimension 0 instead of noise.
