...
impl DepsAnalysis {
    ...
    fn leaf_deps_and_children(node: &ENode) -> (Option<Deps>, Vec<EClassId>) {
        match node {
            ...
            // Tuple
            ENode::Tuple(elems) => (None, elems.clone()),
        }
    }
...
mod tests {
    ...
    fn test_uniform_analysis() {
        ...
        let w_times_2 = eg.add(ENode::Mul(w, two));
        let sin_w = eg.add(ENode::Sin(w_times_2));
    ...
    fn test_mixed_deps() {
        ...
        let w = eg.add(ENode::Var(var::W));
        let two = eg.add(ENode::constant(2.0));
        let w2 = eg.add(ENode::Mul(w, two));
        let sin_w2 = eg.add(ENode::Sin(w2));

        let x = eg.add(ENode::Var(var::X));
        let result = eg.add(ENode::Add(sin_w2, x));
