...
impl GuidedMcts {
    ...
    pub fn run(&mut self) -> GuidedResult {
        ...
        loop {
            ...
            if let Some(timeout) = self.config.timeout && start.elapsed() >= timeout {
                break;
            }

            if let Some(max_iter) = self.config.max_iterations && self.total_iterations >= max_iter {
                break;
            }
