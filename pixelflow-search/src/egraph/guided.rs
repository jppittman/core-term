...
impl GuidedState {
    ...
    fn insert_tree(egraph: &mut EGraph, tree: &ExprTree) -> EClassId {
        match tree {
            ExprTree::Var(v) => egraph.add(ENode::Var(*v)),
            ExprTree::Const(c) => egraph.add(ENode::constant(*c)),
            ExprTree::Add(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Add(a_id, b_id))
            }
            ExprTree::Sub(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Sub(a_id, b_id))
            }
            ExprTree::Mul(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Mul(a_id, b_id))
            }
            ExprTree::Div(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Div(a_id, b_id))
            }
            ExprTree::Neg(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Neg(a_id))
            }
            ExprTree::Sqrt(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Sqrt(a_id))
            }
            ExprTree::Rsqrt(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Rsqrt(a_id))
            }
            ExprTree::Recip(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Recip(a_id))
            }
            ExprTree::Abs(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Abs(a_id))
            }
            ExprTree::Min(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Min(a_id, b_id))
            }
            ExprTree::Max(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Max(a_id, b_id))
            }
            ExprTree::MulAdd(a, b, c) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                let c_id = Self::insert_tree(egraph, c);
                egraph.add(ENode::MulAdd(a_id, b_id, c_id))
            }
            ExprTree::Floor(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Floor(a_id))
            }
            ExprTree::Ceil(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Ceil(a_id))
            }
            ExprTree::Round(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Round(a_id))
            }
            ExprTree::Fract(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Fract(a_id))
            }
            ExprTree::Sin(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Sin(a_id))
            }
            ExprTree::Cos(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Cos(a_id))
            }
            ExprTree::Tan(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Tan(a_id))
            }
            ExprTree::Asin(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Asin(a_id))
            }
            ExprTree::Acos(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Acos(a_id))
            }
            ExprTree::Atan(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Atan(a_id))
            }
            ExprTree::Exp(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Exp(a_id))
            }
            ExprTree::Exp2(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Exp2(a_id))
            }
            ExprTree::Ln(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Ln(a_id))
            }
            ExprTree::Log2(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Log2(a_id))
            }
            ExprTree::Log10(a) => {
                let a_id = Self::insert_tree(egraph, a);
                egraph.add(ENode::Log10(a_id))
            }
            ExprTree::Atan2(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Atan2(a_id, b_id))
            }
            ExprTree::Pow(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Pow(a_id, b_id))
            }
            ExprTree::Hypot(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Hypot(a_id, b_id))
            }
            ExprTree::Lt(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Lt(a_id, b_id))
            }
            ExprTree::Le(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Le(a_id, b_id))
            }
            ExprTree::Gt(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Gt(a_id, b_id))
            }
            ExprTree::Ge(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Ge(a_id, b_id))
            }
            ExprTree::Eq(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Eq(a_id, b_id))
            }
            ExprTree::Ne(a, b) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                egraph.add(ENode::Ne(a_id, b_id))
            }
            ExprTree::Select(a, b, c) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                let c_id = Self::insert_tree(egraph, c);
                egraph.add(ENode::Select(a_id, b_id, c_id))
            }
            ExprTree::Clamp(a, b, c) => {
                let a_id = Self::insert_tree(egraph, a);
                let b_id = Self::insert_tree(egraph, b);
                let c_id = Self::insert_tree(egraph, c);
                egraph.add(ENode::Clamp(a_id, b_id, c_id))
            }
            ExprTree::Tuple(elems) => {
                let elem_ids: Vec<_> = elems
                    .iter()
                    .map(|e| Self::insert_tree(egraph, e))
                    .collect();
                egraph.add(ENode::Tuple(elem_ids))
            }
        }
    }
