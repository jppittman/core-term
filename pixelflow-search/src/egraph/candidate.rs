...
//! is exactly the natural dedup key for that: `(rule_idx, canonical class
...
pub struct Firing {
    /// Index into the e-graph's rule list (`ApplicationRecord::rule_idx`).
    pub rule_idx: usize,
...
mod tests {
    ...
    fn repeated_idempotent_refires_share_a_candidate_key() {
        ...
        let registered_budget = egraph.provenance().application_count();
