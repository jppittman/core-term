...
mod tests {
    ...
    // ========================================================================
    // IncrementalExtractor::extract_choices_only tests
    //
    // `extract_choices_only` had zero test coverage: all 12 mutants that swap
    // its `(f32, Vec<Option<usize>>)` return value for hardcoded garbage went
    // unnoticed (cargo-mutants, 2026-08-10). These tests exercise it through
    // its public entry point (`IncrementalExtractor::new(..).extract_choices_only`)
    // the same way `extract_neural_to_arena` (its sole production caller) does.
    //
    // Two mutants in this function are left un-caught, documented rather than
    // chased with a contrived construction:
    //
    // - `143:34: replace < with <=` (best-improvement acceptance,
    //   `test_cost < best_swap_cost`) only diverges from `<` on an exact
    //   float tie between two *different* candidate subtrees' NNUE-predicted
    //   costs. The NNUE forward pass is not order-invariant even for
    //   commutative operands (`Add(x, y)` and `Add(y, x)` measurably differ,
    //   confirmed empirically), so forcing a bit-identical prediction between
    //   two distinct subtrees would mean reverse-engineering the embedding/
    //   MLP internals rather than testing the extractor's own logic — and the
    //   result would be brittle to any change in the accumulator encoding.
    // - `173:16: delete !` (`if !improved { break; }` → `if improved { break; }`)
    //   only diverges when a pass produces an improvement AND a *later* pass
    //   produces a further, independent improvement — e.g. class Y only
    //   becomes reachable (and swappable) after class X's swap in an earlier
    //   pass backfills it in, per the comment at line 154. Reproducing this
    //   deterministically requires the NNUE to accept two specific,
    //   sequential swaps across every tested seed, which is not controllable
    //   without hand-tuning network weights.
    // ========================================================================

    /// `x + x` and `x * 2` are unioned into one e-class, mirroring
    /// `extract_latency_prior_picks_cheaper_equivalent_form` but through the
    /// NNUE-guided incremental extractor instead of the static cost model.
    #[test]
    fn extract_choices_only_backfills_every_reachable_class_when_equivalent_forms_are_unioned() {
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let two = egraph.add(ENode::constant(2.0));
        let x_plus_x = egraph.add(ENode::Op {
        ...
        assert_eq!(
            choices.len(),
            egraph.num_classes(),
            "choices must be indexed by every e-class the graph has, not a truncated or empty vec"
        );
        assert!(
            cost.is_finite(),
            "predicted cost should be a real number, got {cost}"
        );

        // Backfill invariant: every class reachable from root has Some(_).
        for (class, name) in [
            (root, "root"),
            (egraph.find(x), "x"),
            (egraph.find(two), "two"),
            ...
            assert!(
                choices[class.0 as usize].is_some(),
                "{name}'s e-class ({class:?}) is reachable from root but has no recorded choice"
            );
        }

        // The root choice must be a real node of the root e-class: either the
        // Add form or the Mul form, never a hardcoded garbage index.
        let root_idx = choices[root.0 as usize].unwrap();
        let root_node = &egraph.nodes(root)[root_idx];
        let is_add_or_mul_form = matches!(
            root_node,
            ENode::Op { op, children }
                if children.len() == 2
                    && matches!(op.kind(), pixelflow_ir::OpKind::Add | pixelflow_ir::OpKind::Mul)
        );
        assert!(
            is_add_or_mul_form,
            "root choice should be the Add or Mul form of the unioned e-class, got {root_node:?}"
        );
    }

    /// `X * X`: X is reached twice from the root. Exercises the DAG-aware
    /// ref-counting/backfill path (`get_active_classes`'s dedup) with real
    /// sharing, not just a linear tree.
    #[test]
    fn extract_choices_only_backfills_a_class_reached_through_two_parents() {
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let x_squared = egraph.add(ENode::Op {
            op: &super::super::ops::Mul,
        ...
        let root = egraph.find(x_squared);
        ...
        assert!(
            choices[egraph.find(x).0 as usize].is_some(),
            "X's e-class is reachable from root through both Mul operands but has no recorded choice"
        );
        assert!(
            choices[root.0 as usize].is_some(),
            "root e-class has no recorded choice"
        );
    }

    /// Reproduces the mutual-reference-cycle shape the module's own doc
    /// comments describe (class A picks `neg(B)`, class B could pick
    /// `neg(A)`), but on the refinement loop's candidate-swap path rather
    /// than the bottom-up DP: class B's e-class holds two forms, `neg(p)`
    /// (its bootstrap default) and `neg(a)` (a candidate that would close a
    /// cycle back through A, which currently resolves to `neg(B)`). This is
    /// deterministic regardless of the NNUE's cost prediction because
    /// `choices_have_cycle_from` rejects the candidate *before* any cost is
    /// computed (extract.rs:121-127) — so every seed must reject it.
    #[test]
    fn extract_choices_only_never_adopts_a_swap_that_would_create_a_mutual_reference_cycle() {
        let mut egraph = EGraph::new();
        let p = egraph.add(ENode::Var(0));
        let b_stub = egraph.add(ENode::Op {
            op: &super::super::ops::Neg,
            children: alloc::vec![p],
        });
        let a = egraph.add(ENode::Op {
            op: &super::super::ops::Neg,
            children: alloc::vec![b_stub],
        });
        let b_real = egraph.add(ENode::Op {
            op: &super::super::ops::Neg,
            children: alloc::vec![a],
        });
        // Merges b_stub's class ({neg(p)}) with b_real's ({neg(a)}), giving
        // B two candidate nodes without ever letting A hold more than one
        // (A was never unioned, so it stays forced to neg(B)).
        egraph.union(b_stub, b_real);
        let root = egraph.find(a);
        let b_class = egraph.find(b_stub);

        for seed in [1u64, 2, 42, 100] {
            let nnue = ExprNnue::new_with_latency_prior(seed);
            let extractor = IncrementalExtractor::new(&nnue, 8);
            let (cost, choices) = extractor.extract_choices_only(&egraph, root);

            assert!(
                cost.is_finite(),
                "seed {seed}: predicted cost should be a real number, got {cost}"
            );
            assert_eq!(
                choices[b_class.0 as usize],
                Some(0),
                "seed {seed}: B must keep its non-cyclic neg(p) choice (index 0) — adopting \
                 index 1 (neg(a)) would close a cycle back through A, which is forced to neg(B)"
            );
        }
    }

    /// A class unioned with a node that references itself directly (e.g. a
    /// fold that produces `x = neg(x)`'s e-class holding both `Var` and
    /// `neg(itself)`). The refinement loop's self-reference filter
    /// (extract.rs:115-118) must reject this candidate before ever calling
    /// `choices_have_cycle_from` — a class can never legally choose a node
    /// that points back at its own e-class.
    #[test]
    fn extract_choices_only_skips_a_directly_self_referential_candidate() {
        let mut egraph = EGraph::new();
        let c = egraph.add(ENode::Var(0));
        let neg_self = egraph.add(ENode::Op {
            op: &super::super::ops::Neg,
            children: alloc::vec![c],
        });
        egraph.union(c, neg_self);
        let root = egraph.find(c);
        ...
        for seed in [1u64, 2, 42] {
            ...
            assert_eq!(
                choices[root.0 as usize],
                Some(0),
                "seed {seed}: the class must keep its Var(0) choice (index 0) — the only other \
                 node, neg(itself), directly references its own e-class and must never be chosen"
            );
        }
    }

    // ========================================================================
    // get_active_classes tests (private helper, called from
    // `extract_choices_only`'s refinement loop). Constructed directly since
    // the task's "smaller direct scenario" allowance applies once indirect
    // coverage from the tests above is confirmed insufficient by itself.
    // ========================================================================

    #[test]
    fn get_active_classes_visits_a_shared_child_only_once() {
        // X * X: get_active_classes must not double-count or infinite-loop
        // on the shared X child reached from both Mul operands.
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let x_squared = egraph.add(ENode::Op {
            op: &super::super::ops::Mul,
        ...
        let num_classes = egraph.num_classes();
        let mut choices: Vec<Option<usize>> = alloc::vec![None; num_classes];
        backfill_reachable_defaults(&egraph, root, &mut choices);

        let nnue = ExprNnue::new_with_latency_prior(1);
        let extractor = IncrementalExtractor::new(&nnue, 8);
        let active = extractor.get_active_classes(&egraph, root, &choices);

        assert_eq!(
            active,
            alloc::vec![root, egraph.find(x)],
            "active classes for X * X should be exactly [root, X], X visited once despite two references"
        );
    }

    #[test]
    fn get_active_classes_skips_a_recorded_choice_that_is_out_of_bounds_for_the_eclass() {
        // A leaf class (one node) with a recorded choice pointing past its
        // single node. get_active_classes must not index nodes[node_idx]
        // (would panic) and must still report the class itself as active.
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let root = egraph.find(x);
        let choices: Vec<Option<usize>> = alloc::vec![Some(1)]; // nodes.len() == 1
        ...
        assert_eq!(
            active,
            alloc::vec![root],
            "an out-of-bounds recorded choice must not panic and must not fabricate children"
        );
    }

    // ========================================================================
    // backfill_reachable_defaults tests (private helper).
    // ========================================================================

    #[test]
    fn backfill_reachable_defaults_fills_every_reachable_class_with_its_first_node() {
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let y = egraph.add(ENode::Var(1));
        let sum = egraph.add(ENode::Op {
        ...
        let num_classes = egraph.num_classes();
        let mut choices: Vec<Option<usize>> = alloc::vec![None; num_classes];
        backfill_reachable_defaults(&egraph, sum, &mut choices);

        assert_eq!(
            choices[egraph.find(sum).0 as usize],
            Some(0),
            "sum should default to node 0"
        );
        assert_eq!(
            choices[egraph.find(x).0 as usize],
            Some(0),
            "x should default to node 0"
        );
        assert_eq!(
            choices[egraph.find(y).0 as usize],
            Some(0),
            "y should default to node 0"
        );
    }

    #[test]
    fn backfill_reachable_defaults_leaves_an_already_recorded_choice_untouched() {
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));

        let num_classes = egraph.num_classes();
        let mut choices: Vec<Option<usize>> = alloc::vec![None; num_classes];
        choices[egraph.find(x).0 as usize] = Some(99); // sentinel, not a real node index

        backfill_reachable_defaults(&egraph, x, &mut choices);

        assert_eq!(
            choices[egraph.find(x).0 as usize],
            Some(99),
            "a class that already has a recorded choice must not be overwritten with the default"
        );
    }

    #[test]
    fn backfill_reachable_defaults_skips_a_class_id_outside_the_provided_choices_slice() {
        let mut egraph = EGraph::new();
        let x = egraph.add(ENode::Var(0));
        let y = egraph.add(ENode::Var(1));
        let sum = egraph.add(ENode::Op {
        ...
        assert!(
            egraph.find(sum).0 as usize >= 2,
            "test assumes sum's canonical id is >= 2 so it falls outside a 2-slot choices vec"
        );

        // Deliberately shorter than egraph.num_classes(): backfill must treat
        // its own `choices.len()` as the bound, not egraph.num_classes().
        let mut choices: Vec<Option<usize>> = alloc::vec![None; 2];
        backfill_reachable_defaults(&egraph, sum, &mut choices);

        assert_eq!(
            choices,
            alloc::vec![None, None],
            "a class id outside the provided slice must be skipped, not indexed out of bounds"
        );
    }

    // ========================================================================
    // break_choice_cycles tests, exercised only through the public extract()
    // entry point (extract.rs:591 calls it internally) — matching this
    // module's convention of never calling its own private helpers from
    // outside their natural call path.
    //
    // A genuine DP-forced mutual cycle (class A's only node is neg(B), class
    // B's only *reachable-cost* node is neg(A)) cannot be built from an empty
    // e-graph with real per-op costs: constructing forward references always
    // requires a placeholder leaf to seed the first node, and that placeholder
    // remains a strictly-cheaper escape once unioned in, so the plain
    // non-negative-cost DP never willingly picks the cyclic option. Forcing it
    // requires a cost function where leaves are the expensive choice and ops
    // are free — legitimate given `extract`'s `CostFunction` is a pluggable
    // trait, and it reproduces exactly the "class 68 picks neg(69), class 69
    // picks neg(68)" scenario the module's own doc comments describe.
    // ========================================================================

    /// Inverts the usual cost ordering: leaves are expensive, ops are free.
    /// Combined with `extract`'s cycle-cost poisoning (any node whose cost
    /// depends on a not-yet-computed sibling is charged `CYCLE_COST`, a
    /// large but finite 1_000_000), this makes the DP prefer a node that
    /// depends on itself (cost ~1_000_000) over a real leaf (cost 2_000_000)
    /// — forcing a genuine mutual-reference choice for the DP to break.
    struct InvertedCost;

    impl CostFunction for InvertedCost {
        fn node_cost(&self, node: &ENode, _parent: Option<pixelflow_ir::OpKind>) -> usize {
            match node {
                ENode::Var(_) | ENode::Const(_) | ENode::Buffer(_) => 2_000_000,
                ENode::Op { .. } => 0,
            }
        }
    }
    ...
    #[test]
    fn extract_breaks_a_mutual_reference_cycle_forced_by_an_inverted_cost_function() {
        let mut egraph = EGraph::new();
        let p = egraph.add(ENode::Var(0));
        let b_stub = egraph.add(ENode::Op {
            op: &super::super::ops::Neg,
        ...
        let costs = InvertedCost;
        // Success here (no panic, no hang) is itself the strong signal: an
        // unbroken cycle would make `choices_to_arena`'s traversal revisit a
        // class before it is cached and recurse forever.
        let (arena, root_id, _cost) = extract(&egraph, root, &costs);

        assert_eq!(
            arena.len(),
            3,
            "the broken-cycle expression should materialise as exactly \
             Var(p), Neg(p), Neg(Neg(p)) — three arena nodes"
        );
        assert!(matches!(
            arena.node(root_id),
            pixelflow_ir::arena::ExprNode::Unary(pixelflow_ir::OpKind::Neg, _)
        ));
    }
}
