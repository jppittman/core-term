...
/// THE class-cap ghost measurement (2026-09-02,
/// docs/results/2026-09-02-class-cap-ghosts.md): `EGraph::union` never
/// removes an entry from `self.classes` (`graph.rs::union`), so
/// `num_classes()` (`self.classes.len()`) counts every class ever
/// ALLOCATED, monotonically increasing, while the live graph is
/// `class_ids()` — canonical AND non-empty. The production budget check
/// (`graph.rs::saturate_bounded`, `self.classes.len() > max_classes`) is
/// checked against the allocated count, not the live one. This module
/// quantifies the gap on the 206-real-kernel corpus (glyph/shader/
/// psychedelic/cell-grid dumps from `congruence_gap_probe`'s corpus,
/// `PIXELFLOW_CLASSCAP_ARENA_DIR`) plus a size-stratified synthetic sample,
/// and measures the upper-bound extracted-cost recovery from raising the
/// cap to `5000 * (allocated/live ratio observed at the original cap-hit)`.
///
/// Read-only with respect to production: never changes `union`,
/// `saturate_bounded`, or the default `Budget::Production` cap. The only
/// non-test change this measurement required was `EGraph::memo_len()`
