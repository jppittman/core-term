...
/// Every method name the front end advertises must actually lower.
///
/// The bug this exists to prevent shipped once already: `sema` accepted
/// `.round()`, `.log10()` and `.pow()` (they are ordinary `OpKind`s, so
/// `known_method_names()` returned them) while this module's dispatch had no
/// arm for any of the three, so a kernel body calling one compiled through
/// validation and optimization and then failed at arena lowering. The
/// mirror-image gap hid `fract`/`hypot`/`clamp`: the e-graph could decompose
/// them, but `sema` rejected the names first, leaving that decomposition
/// unreachable from either macro.
///
/// No sampled test can catch that class. A method is only exercised if some
/// test happens to write a kernel that calls it, and `kernel_macro.rs`'s
/// cases are hand-picked — every op below was covered at the SIMD backend,
/// in codegen, and on the `Kernel` value API, and still nobody had written
/// `X.round()` inside a `kernel!` body. So sweep the advertised surface
/// exhaustively instead: adding an op to `OpKind::is_dsl_method` without a
/// lowering path fails here immediately, for whoever adds it.
#[cfg(test)]
mod every_advertised_method_lowers {
    use super::{LIBRARY_METHODS, ast_to_arena};
    use crate::ast::{Expr, IdentExpr, MethodCallExpr};
    ...
    /// The `X` coordinate — a receiver and argument that always lowers.
    fn x() -> Expr {
    ...
    /// Lower `X.<method>(X, ..)` with `arg_count` arguments.
    fn lower(method: &str, arg_count: usize) -> Result<(), String> {
        let call = Expr::MethodCall(MethodCallExpr {
            receiver: Box::new(x()),
            method: Ident::new(method, Span::call_site()),
            args: (0..arg_count).map(|_| x()).collect(),
            span: Span::call_site(),
        });
        ast_to_arena(&call, &HashMap::new(), &mut ExprArena::new()).map(|_| ())
    }

    #[test]
    fn every_primitive_op_name_sema_accepts_reaches_the_arena() {
        for name in known_method_names() {
            ...
            let arg_count = op.arity() - 1;
            assert_eq!(
                lower(name, arg_count),
                Ok(()),
                "`.{name}()` is advertised by known_method_names(), so sema accepts it, \
                 but arena lowering has no path for it"
            );
        }
    }

    #[test]
    fn every_library_method_sema_accepts_reaches_the_arena() {
        for &(name, arg_count) in LIBRARY_METHODS {
            assert_eq!(
                lower(name, arg_count),
                Ok(()),
                "`.{name}()` is in LIBRARY_METHODS, so sema accepts it, \
                 but arena lowering has no path for it"
            );
        }
    }
    ...
    /// so the sweep above can't be satisfied by making lowering accept
    /// everything.
    #[test]
    fn a_name_the_front_end_does_not_advertise_is_still_refused() {
        assert!(lower("not_a_real_method", 0).is_err());
        // A real op at the wrong arity is just as unadvertised.
        assert!(lower("sqrt", 2).is_err());
    }
}
