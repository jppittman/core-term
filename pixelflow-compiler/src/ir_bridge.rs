    ...
    let node_tokens: Vec<TokenStream> = nodes
        ...
        .map(|node| match node {
            ...
            pixelflow_ir::arena::ExprNode::Const(v) => {
                let bits = v.to_bits();
                quote! { ::pixelflow_ir::arena::ExprNode::Const(f32::from_bits(#bits)) }
            }
...
pub fn composition_stmts(plan: &CompositionPlan, accessors: &[TokenStream]) -> TokenStream {
            ...
            quote! {
                {
                    ...
                    let __warped = __arena.substitute_vars_with(
                        __frag,
                        &[
                            (0u8, ::pixelflow_ir::arena::ExprId(#cx)),
                            (1u8, ::pixelflow_ir::arena::ExprId(#cy)),
                            (2u8, ::pixelflow_ir::arena::ExprId(#cz)),
                            (3u8, ::pixelflow_ir::arena::ExprId(#cw)),
                        ],
                    );
    ...
    quote! {
        let mut __subs: ::std::vec::Vec<(u8, ::pixelflow_ir::arena::ExprId)> =
            ::std::vec::Vec::new();
        #( #bare )*
        #( #sites )*
...
fn opkind_to_tokens(kind: OpKind) -> TokenStream {
    match kind {
        ...
        OpKind::BitAnd => quote! { ::pixelflow_ir::OpKind::BitAnd },
        OpKind::BitOr => quote! { ::pixelflow_ir::OpKind::BitOr },
