    ...
    match ir {
        ...
        IR::Unary(op, child) => {
            ...
            match op {
                ...
                OpKind::Floor => quote! { (#child_code).floor() },
                OpKind::Ceil => quote! { (#child_code).ceil() },
                OpKind::Round => quote! { (#child_code).round() },
                OpKind::Fract => quote! { (#child_code).fract() },
                OpKind::Sin => quote! { (#child_code).sin() },
                OpKind::Cos => quote! { (#child_code).cos() },
                OpKind::Tan => quote! { (#child_code).tan() },
                OpKind::Asin => quote! { (#child_code).asin() },
                OpKind::Acos => quote! { (#child_code).acos() },
                OpKind::Atan => quote! { (#child_code).atan() },
                OpKind::Exp => quote! { (#child_code).exp() },
                OpKind::Exp2 => quote! { (#child_code).exp2() },
                OpKind::Ln => quote! { (#child_code).ln() },
                OpKind::Log2 => quote! { (#child_code).log2() },
                OpKind::Log10 => quote! { (#child_code).log10() },
        ...
        IR::Binary(op, lhs, rhs) => {
            ...
            match op {
                ...
                OpKind::Atan2 => quote! { (#lhs_code).atan2(#rhs_code) },
                OpKind::Pow => quote! { (#lhs_code).powf(#rhs_code) },
                OpKind::Hypot => quote! { (#lhs_code).hypot(#rhs_code) },
        ...
        IR::Ternary(op, a, b, c) => {
            ...
            match op {
                ...
                OpKind::Clamp => quote! { (#a_code).clamp(#b_code, #c_code) },
                OpKind::Select => quote! { (#a_code).select(#b_code, #c_code) },
