...
///
/// This macro compiles expressions to native machine code at runtime using
/// our custom JIT emitter, completely bypassing LLVM's slow compilation.
///
/// # Why Use This?
///
/// - **Fast compilation**: JIT compilation takes microseconds vs minutes for LLVM
/// - **Runtime loading**: Load kernels from files, network, user input
/// - **No monomorphization explosion**: 10K kernels don't mean 10K types
///
/// # Trade-offs
///
/// - ~80-90% of LLVM's optimization quality (no advanced loop opts, etc.)
/// - First call has JIT overhead (cached after that)
/// - Only supports simple expressions (no captured parameters yet)
///
/// # Example
///
/// ```ignore
...
///
/// let kernel = kernel_jit!(|| X + Y);
///
/// // Call with SIMD vectors
/// use core::arch::aarch64::*;
/// let x = vdupq_n_f32(1.0);
/// let y = vdupq_n_f32(2.0);
/// let z = vdupq_n_f32(0.0);
/// let w = vdupq_n_f32(0.0);
/// let result = kernel(x, y, z, w);  // Returns 3.0 in all lanes
/// ```
#[proc_macro]
pub fn kernel_jit(input: TokenStream) -> TokenStream {
    ...
    // Phase 4: Generate runtime Expr constructor
    let expr_code = ir_bridge::ir_to_runtime_expr(&ir);

    // Phase 5: Emit JIT compilation wrapper
    let output = quote! {
        {
            use ::std::sync::OnceLock;

            static KERNEL: OnceLock<::pixelflow_ir::backend::emit::executable::ExecutableCode> =
                OnceLock::new();

            let exec = KERNEL.get_or_init(|| {
                let expr = #expr_code;
                ::pixelflow_ir::backend::emit::compile(&expr)
                    .expect("JIT compilation failed")
            });

            unsafe {
                exec.as_fn::<::pixelflow_ir::backend::emit::executable::KernelFn>()
            }
        }
    };
